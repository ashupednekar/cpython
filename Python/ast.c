...
static int validate_exprs(struct validator *, asdl_expr_seq *, expr_context_ty,
                          int);
...
#define VALIDATE_POSITIONS(node)                                               \
  if (node->lineno > node->end_lineno) {                                       \
    PyErr_Format(PyExc_ValueError,                                             \
                 "AST node line range (%d, %d) is not valid", node->lineno,    \
                 node->end_lineno);                                            \
    return 0;                                                                  \
  }                                                                            \
  ...
  if (node->lineno == node->end_lineno &&                                      \
    ...
    PyErr_Format(PyExc_ValueError,                                             \
                 "line %d, column %d-%d is not a valid range", node->lineno,   \
                 node->col_offset, node->end_col_offset);                      \
    return 0;                                                                  \
  }

static int validate_name(PyObject *name) {
...
static int validate_comprehension(struct validator *state,
                                  asdl_comprehension_seq *gens) {
  assert(!PyErr_Occurred());
...
static int validate_keywords(struct validator *state,
...
static int validate_args(struct validator *state, asdl_arg_seq *args) {
  assert(!PyErr_Occurred());
...
static const char *expr_context_name(expr_context_ty ctx) {
  switch (ctx) {
  case Load:
    return "Load";
  case Store:
...
static int validate_arguments(struct validator *state, arguments_ty args) {
  assert(!PyErr_Occurred());
  if (!validate_args(state, args->posonlyargs) ||
      !validate_args(state, args->args)) {
    return 0;
  }
...
static int validate_constant(struct validator *state, PyObject *value) {
  assert(!PyErr_Occurred());
  ...
  if (!PyErr_Occurred()) {
    PyErr_Format(PyExc_TypeError, "got an invalid type in Constant: %s",
                 _PyType_Name(Py_TYPE(value)));
  }
  return 0;
}

static int validate_expr(struct validator *state, expr_ty exp,
                         expr_context_ty ctx) {
  assert(!PyErr_Occurred());
...
#undef COMP
  ...
  case Slice_kind:
    ret = (!exp->v.Slice.lower ||
           validate_expr(state, exp->v.Slice.lower, Load)) &&
          (!exp->v.Slice.upper ||
           validate_expr(state, exp->v.Slice.upper, Load)) &&
...
static int ensure_literal_number(expr_ty exp, bool allow_real,
...
static int ensure_literal_negative(expr_ty exp, bool allow_real,
                                   bool allow_imaginary) {
...
static int ensure_literal_complex(expr_ty exp) {
...
static int validate_pattern_match_value(struct validator *state, expr_ty exp) {
  assert(!PyErr_Occurred());
...
static int validate_capture(PyObject *name) {
  assert(!PyErr_Occurred());
...
static int validate_pattern(struct validator *state, pattern_ty p,
                            int star_ok) {
  assert(!PyErr_Occurred());
  ...
  case MatchOr_kind:
    if (asdl_seq_LEN(p->v.MatchOr.patterns) < 2) {
      PyErr_SetString(PyExc_ValueError, "MatchOr requires at least 2 patterns");
      ret = 0;
      break;
    }
...
static int _validate_nonempty_seq(asdl_seq *seq, const char *what,
                                  const char *owner) {
  if (asdl_seq_LEN(seq))
    return 1;
...
static int validate_assignlist(struct validator *state, asdl_expr_seq *targets,
                               expr_context_ty ctx) {
  assert(!PyErr_Occurred());
  return validate_nonempty_seq(targets, "targets",
                               ctx == Del ? "Delete" : "Assign") &&
         validate_exprs(state, targets, ctx, 0);
}

static int validate_body(struct validator *state, asdl_stmt_seq *body,
                         const char *owner) {
  assert(!PyErr_Occurred());
  return validate_nonempty_seq(body, "body", owner) &&
         validate_stmts(state, body);
}

static int validate_stmt(struct validator *state, stmt_ty stmt) {
  assert(!PyErr_Occurred());
  ...
  case AsyncFunctionDef_kind:
    ret = validate_body(state, stmt->v.AsyncFunctionDef.body,
                        "AsyncFunctionDef") &&
          validate_type_params(state, stmt->v.AsyncFunctionDef.type_params) &&
          validate_arguments(state, stmt->v.AsyncFunctionDef.args) &&
          validate_exprs(state, stmt->v.AsyncFunctionDef.decorator_list, Load,
                         0) &&
...
static int validate_stmts(struct validator *state, asdl_stmt_seq *seq) {
  assert(!PyErr_Occurred());
...
static int validate_exprs(struct validator *state, asdl_expr_seq *exprs,
                          expr_context_ty ctx, int null_ok) {
  assert(!PyErr_Occurred());
...
static int validate_patterns(struct validator *state,
                             asdl_pattern_seq *patterns, int star_ok) {
  assert(!PyErr_Occurred());
...
static int validate_typeparam(struct validator *state, type_param_ty tp) {
...
static int validate_type_params(struct validator *state,
...
int _PyAST_Validate(mod_ty mod) {
  ...
  case FunctionType_kind:
    res = validate_exprs(&state, mod->v.FunctionType.argtypes, Load,
                         /*null_ok=*/0) &&
