...
static int init_identifiers(struct ast_state *state) {
  ...
  if ((state->__doc__ = PyUnicode_InternFromString("__doc__")) == NULL)
    return -1;
  if ((state->__match_args__ = PyUnicode_InternFromString("__match_args__")) ==
      NULL)
    return -1;
  ...
  if ((state->comparators = PyUnicode_InternFromString("comparators")) == NULL)
    return -1;
  if ((state->context_expr = PyUnicode_InternFromString("context_expr")) ==
      NULL)
    return -1;
  ...
  if ((state->ctx = PyUnicode_InternFromString("ctx")) == NULL)
    return -1;
  if ((state->decorator_list = PyUnicode_InternFromString("decorator_list")) ==
      NULL)
    return -1;
  if ((state->default_value = PyUnicode_InternFromString("default_value")) ==
      NULL)
    return -1;
  ...
  if ((state->elts = PyUnicode_InternFromString("elts")) == NULL)
    return -1;
  if ((state->end_col_offset = PyUnicode_InternFromString("end_col_offset")) ==
      NULL)
    return -1;
  ...
  if ((state->kwd_attrs = PyUnicode_InternFromString("kwd_attrs")) == NULL)
    return -1;
  if ((state->kwd_patterns = PyUnicode_InternFromString("kwd_patterns")) ==
      NULL)
    return -1;
  ...
  if ((state->ops = PyUnicode_InternFromString("ops")) == NULL)
    return -1;
  if ((state->optional_vars = PyUnicode_InternFromString("optional_vars")) ==
      NULL)
    return -1;
  ...
  if ((state->type = PyUnicode_InternFromString("type")) == NULL)
    return -1;
  if ((state->type_comment = PyUnicode_InternFromString("type_comment")) ==
      NULL)
    return -1;
  if ((state->type_ignores = PyUnicode_InternFromString("type_ignores")) ==
      NULL)
    return -1;
...
static PyObject *ast2obj_stmt(struct ast_state *state, struct validator *vstate,
                              void *);
...
static PyObject *ast2obj_expr(struct ast_state *state, struct validator *vstate,
                              void *);
...
static PyObject *ast2obj_expr_context(struct ast_state *state,
                                      struct validator *vstate,
                                      expr_context_ty);
static PyObject *ast2obj_boolop(struct ast_state *state,
                                struct validator *vstate, boolop_ty);
static PyObject *ast2obj_operator(struct ast_state *state,
                                  struct validator *vstate, operator_ty);
static PyObject *ast2obj_unaryop(struct ast_state *state,
                                 struct validator *vstate, unaryop_ty);
static PyObject *ast2obj_cmpop(struct ast_state *state,
                               struct validator *vstate, cmpop_ty);
static PyObject *ast2obj_comprehension(struct ast_state *state,
                                       struct validator *vstate, void *);
...
static PyObject *ast2obj_excepthandler(struct ast_state *state,
                                       struct validator *vstate, void *);
...
static PyObject *ast2obj_arguments(struct ast_state *state,
                                   struct validator *vstate, void *);
...
static PyObject *ast2obj_keyword(struct ast_state *state,
                                 struct validator *vstate, void *);
...
static PyObject *ast2obj_alias(struct ast_state *state,
                               struct validator *vstate, void *);
...
static PyObject *ast2obj_withitem(struct ast_state *state,
                                  struct validator *vstate, void *);
static const char *const withitem_fields[] = {
    "context_expr",
    "optional_vars",
};
static PyObject *ast2obj_match_case(struct ast_state *state,
                                    struct validator *vstate, void *);
...
static PyObject *ast2obj_pattern(struct ast_state *state,
                                 struct validator *vstate, void *);
...
static PyObject *ast2obj_type_ignore(struct ast_state *state,
                                     struct validator *vstate, void *);
...
static PyObject *ast2obj_type_param(struct ast_state *state,
                                    struct validator *vstate, void *);
...
static int add_ast_annotations(struct ast_state *state) {
  bool cond;
    ...
    return 0;
  {
    ...
    cond =
        PyDict_SetItemString(FunctionType_annotations, "argtypes", type) == 0;
    Py_DECREF(type);
    if (!cond) {
      Py_DECREF(FunctionType_annotations);
      return 0;
  ...
  }
  {
    ...
    cond =
        PyDict_SetItemString(FunctionDef_annotations, "type_params", type) == 0;
    Py_DECREF(type);
    if (!cond) {
      Py_DECREF(FunctionDef_annotations);
      return 0;
    ...
    return 0;
  {
    ...
    Py_INCREF(type);
    cond =
        PyDict_SetItemString(AsyncFunctionDef_annotations, "name", type) == 0;
    Py_DECREF(type);
    if (!cond) {
  ...
  }
  {
    ...
    Py_INCREF(type);
    cond =
        PyDict_SetItemString(AsyncFunctionDef_annotations, "args", type) == 0;
    Py_DECREF(type);
    if (!cond) {
      Py_DECREF(AsyncFunctionDef_annotations);
      return 0;
  ...
  }
  {
    ...
    cond =
        PyDict_SetItemString(AsyncFunctionDef_annotations, "body", type) == 0;
    Py_DECREF(type);
    if (!cond) {
      Py_DECREF(AsyncFunctionDef_annotations);
      return 0;
  ...
  }
  {
    ...
    cond = PyDict_SetItemString(AsyncFunctionDef_annotations, "decorator_list",
                                type) == 0;
    Py_DECREF(type);
    if (!cond) {
      Py_DECREF(AsyncFunctionDef_annotations);
  ...
  }
  {
    ...
    cond = PyDict_SetItemString(AsyncFunctionDef_annotations, "type_comment",
                                type) == 0;
    Py_DECREF(type);
    if (!cond) {
      Py_DECREF(AsyncFunctionDef_annotations);
  ...
  }
  {
    ...
    cond = PyDict_SetItemString(AsyncFunctionDef_annotations, "type_params",
                                type) == 0;
    Py_DECREF(type);
    if (!cond) {
      Py_DECREF(AsyncFunctionDef_annotations);
  ...
  }
  {
    ...
    cond =
        PyDict_SetItemString(ClassDef_annotations, "decorator_list", type) == 0;
    Py_DECREF(type);
    if (!cond) {
      Py_DECREF(ClassDef_annotations);
      return 0;
  ...
  }
  {
    ...
    cond =
        PyDict_SetItemString(TypeAlias_annotations, "type_params", type) == 0;
    Py_DECREF(type);
    if (!cond) {
      Py_DECREF(TypeAlias_annotations);
      return 0;
  ...
  }
  {
    ...
    cond =
        PyDict_SetItemString(AsyncFor_annotations, "type_comment", type) == 0;
    Py_DECREF(type);
    if (!cond) {
      Py_DECREF(AsyncFor_annotations);
      return 0;
  ...
  }
  {
    ...
    cond =
        PyDict_SetItemString(AsyncWith_annotations, "type_comment", type) == 0;
    Py_DECREF(type);
    if (!cond) {
      Py_DECREF(AsyncWith_annotations);
      return 0;
  ...
  }
  {
    ...
    cond =
        PyDict_SetItemString(GeneratorExp_annotations, "generators", type) == 0;
    Py_DECREF(type);
    if (!cond) {
      Py_DECREF(GeneratorExp_annotations);
      return 0;
  ...
  cond = PyObject_SetAttrString(state->FormattedValue_type, "__annotations__",
                                FormattedValue_annotations) == 0;
  if (!cond) {
    Py_DECREF(FormattedValue_annotations);
    return 0;
  ...
  }
  {
    ...
    Py_INCREF(type);
    cond =
        PyDict_SetItemString(comprehension_annotations, "is_async", type) == 0;
    Py_DECREF(type);
    if (!cond) {
      Py_DECREF(comprehension_annotations);
      return 0;
    ...
    return 0;
  {
    ...
    cond =
        PyDict_SetItemString(arguments_annotations, "posonlyargs", type) == 0;
    Py_DECREF(type);
    if (!cond) {
      Py_DECREF(arguments_annotations);
      return 0;
  ...
  }
  {
    ...
    cond =
        PyDict_SetItemString(arguments_annotations, "kw_defaults", type) == 0;
    Py_DECREF(type);
    if (!cond) {
      Py_DECREF(arguments_annotations);
      return 0;
    ...
    return 0;
  {
    ...
    Py_INCREF(type);
    cond =
        PyDict_SetItemString(withitem_annotations, "context_expr", type) == 0;
    Py_DECREF(type);
    if (!cond) {
  ...
  }
  {
    ...
    cond =
        PyDict_SetItemString(withitem_annotations, "optional_vars", type) == 0;
    Py_DECREF(type);
    if (!cond) {
      Py_DECREF(withitem_annotations);
      return 0;
  ...
  cond = PyObject_SetAttrString(state->MatchSingleton_type, "__annotations__",
                                MatchSingleton_annotations) == 0;
  if (!cond) {
    Py_DECREF(MatchSingleton_annotations);
    return 0;
    ...
    return 0;
  {
    ...
    cond =
        PyDict_SetItemString(MatchSequence_annotations, "patterns", type) == 0;
    Py_DECREF(type);
    if (!cond) {
      Py_DECREF(MatchSequence_annotations);
      return 0;
  ...
  }
  {
    ...
    cond =
        PyDict_SetItemString(MatchMapping_annotations, "patterns", type) == 0;
    Py_DECREF(type);
    if (!cond) {
      Py_DECREF(MatchMapping_annotations);
      return 0;
  ...
  }
  {
    ...
    cond =
        PyDict_SetItemString(MatchClass_annotations, "kwd_patterns", type) == 0;
    Py_DECREF(type);
    if (!cond) {
      Py_DECREF(MatchClass_annotations);
      return 0;
  ...
  }
  {
    ...
    cond =
        PyDict_SetItemString(TypeVar_annotations, "default_value", type) == 0;
    Py_DECREF(type);
    if (!cond) {
      Py_DECREF(TypeVar_annotations);
      return 0;
  ...
  }
  {
    ...
    cond =
        PyDict_SetItemString(ParamSpec_annotations, "default_value", type) == 0;
    Py_DECREF(type);
    if (!cond) {
      Py_DECREF(ParamSpec_annotations);
      return 0;
...
static int ast_traverse(AST_object *self, visitproc visit, void *arg) {
  Py_VISIT(Py_TYPE(self));
  Py_VISIT(self->dict);
  return 0;
}

static int ast_clear(AST_object *self) {
  Py_CLEAR(self->dict);
  return 0;
}

static int ast_type_init(PyObject *self, PyObject *args, PyObject *kw) {
  ...
  if (numfields < PyTuple_GET_SIZE(args)) {
    PyErr_Format(PyExc_TypeError,
                 ...
                 _PyType_Name(Py_TYPE(self)), numfields,
                 numfields == 1 ? "" : "s");
    res = -1;
    goto cleanup;
  }
  ...
  if (kw) {
    ...
    while (PyDict_Next(kw, &i, &key, &value)) {
      ...
      } else if (PyUnicode_CompareWithASCIIString(key, "lineno") != 0 &&
  ...
  if (size > 0) {
    if (PyObject_GetOptionalAttr((PyObject *)Py_TYPE(self),
                                 &_Py_ID(_field_types), &field_types) < 0) {
      res = -1;
      goto cleanup;
    }
    if (field_types == NULL) {
...
static PyObject *ast_type_reduce(PyObject *self, PyObject *unused) {
  ...
  if (dict) {
    ...
    if (fields) {
      ...
      result = Py_BuildValue("ONO", Py_TYPE(self), args_tuple, remaining_dict);
    } else {
...
static PyMethodDef ast_type_methods[] = {
    {"__reduce__", ast_type_reduce, METH_NOARGS, NULL}, {NULL}};
...
static PyType_Spec AST_type_spec = {"ast.AST", sizeof(AST_object), 0,
                                    Py_TPFLAGS_DEFAULT | Py_TPFLAGS_BASETYPE |
                                        Py_TPFLAGS_HAVE_GC,
                                    AST_type_slots};

static PyObject *make_type(struct ast_state *state, const char *type,
                           PyObject *base, const char *const *fields,
                           int num_fields, const char *doc) {
  ...
  result = PyObject_CallFunction(
      (PyObject *)&PyType_Type, "s(O){OOOOOOOs}", type, base, state->_fields,
      fnames, state->__match_args__, fnames, state->__module__, state->ast,
      state->__doc__, doc);
  Py_DECREF(fnames);
  return result;
}

static int add_attributes(struct ast_state *state, PyObject *type,
...
static PyObject *
ast2obj_list(struct ast_state *state, struct validator *vstate, asdl_seq *seq,
             PyObject *(*func)(struct ast_state *state,
                               struct validator *vstate, void *)) {
...
static PyObject *ast2obj_object(struct ast_state *Py_UNUSED(state),
                                struct validator *Py_UNUSED(vstate), void *o) {
...
static PyObject *ast2obj_int(struct ast_state *Py_UNUSED(state),
                             struct validator *Py_UNUSED(vstate), long b) {
...
static int obj2ast_identifier(struct ast_state *state, PyObject *obj,
                              PyObject **out, PyArena *arena) {
...
static int obj2ast_string(struct ast_state *state, PyObject *obj,
                          PyObject **out, PyArena *arena) {
...
static int obj2ast_int(struct ast_state *Py_UNUSED(state), PyObject *obj,
                       int *out, PyArena *arena) {
  int i;
  if (!PyLong_Check(obj)) {
...
static int add_ast_fields(struct ast_state *state) {
  ...
  if (!empty_tuple ||
      ...
      PyObject_SetAttrString(state->AST_type, "__match_args__", empty_tuple) <
          0 ||
...
static int init_types(struct ast_state *state) {
  ...
  state->mod_type =
      make_type(state, "mod", state->AST_type, NULL, 0,
  ...
  if (add_attributes(state, state->mod_type, NULL, 0) < 0)
    return -1;
  state->Module_type =
      make_type(state, "Module", state->mod_type, Module_fields, 2,
                "Module(stmt* body, type_ignore* type_ignores)");
  if (!state->Module_type)
    return -1;
  state->Interactive_type =
      make_type(state, "Interactive", state->mod_type, Interactive_fields, 1,
                "Interactive(stmt* body)");
  if (!state->Interactive_type)
    return -1;
  state->Expression_type =
      make_type(state, "Expression", state->mod_type, Expression_fields, 1,
                "Expression(expr body)");
  if (!state->Expression_type)
    return -1;
  state->FunctionType_type =
      make_type(state, "FunctionType", state->mod_type, FunctionType_fields, 2,
                "FunctionType(expr* argtypes, expr returns)");
  if (!state->FunctionType_type)
    return -1;
  state->stmt_type = make_type(
      state, "stmt", state->AST_type, NULL, 0,
      ...
      "expr* decorator_list, expr? returns, string? type_comment, type_param* "
      "type_params)\n"
      "     | ClassDef(identifier name, expr* bases, keyword* keywords, stmt* "
      "body, expr* decorator_list, type_param* type_params)\n"
      "     | Return(expr? value)\n"
      "     | Delete(expr* targets)\n"
  ...
  if (PyObject_SetAttr(state->stmt_type, state->end_col_offset, Py_None) == -1)
    return -1;
  state->FunctionDef_type =
      make_type(state, "FunctionDef", state->stmt_type, FunctionDef_fields, 7,
  ...
  state->AsyncFunctionDef_type = make_type(
      state, "AsyncFunctionDef", state->stmt_type, AsyncFunctionDef_fields, 7,
  ...
  if (PyObject_SetAttr(state->AsyncFunctionDef_type, state->type_comment,
    ...
    return -1;
  state->ClassDef_type =
      make_type(state, "ClassDef", state->stmt_type, ClassDef_fields, 6,
  ...
  state->Return_type = make_type(state, "Return", state->stmt_type,
                                 Return_fields, 1, "Return(expr? value)");
  ...
  state->Delete_type = make_type(state, "Delete", state->stmt_type,
                                 Delete_fields, 1, "Delete(expr* targets)");
  if (!state->Delete_type)
    return -1;
  state->Assign_type =
      make_type(state, "Assign", state->stmt_type, Assign_fields, 3,
                "Assign(expr* targets, expr value, string? type_comment)");
  if (!state->Assign_type)
    return -1;
  if (PyObject_SetAttr(state->Assign_type, state->type_comment, Py_None) == -1)
    return -1;
  state->TypeAlias_type =
      make_type(state, "TypeAlias", state->stmt_type, TypeAlias_fields, 3,
                "TypeAlias(expr name, type_param* type_params, expr value)");
  if (!state->TypeAlias_type)
    return -1;
  state->AugAssign_type =
      make_type(state, "AugAssign", state->stmt_type, AugAssign_fields, 3,
                "AugAssign(expr target, operator op, expr value)");
  if (!state->AugAssign_type)
    return -1;
  state->AnnAssign_type = make_type(
      state, "AnnAssign", state->stmt_type, AnnAssign_fields, 4,
  ...
  if (PyObject_SetAttr(state->For_type, state->type_comment, Py_None) == -1)
    return -1;
  state->AsyncFor_type =
      make_type(state, "AsyncFor", state->stmt_type, AsyncFor_fields, 5,
                "AsyncFor(expr target, expr iter, stmt* body, stmt* orelse, "
                "string? type_comment)");
  ...
  if (PyObject_SetAttr(state->AsyncFor_type, state->type_comment, Py_None) ==
    ...
    return -1;
  state->While_type =
      make_type(state, "While", state->stmt_type, While_fields, 3,
                "While(expr test, stmt* body, stmt* orelse)");
  if (!state->While_type)
    return -1;
  ...
  if (!state->If_type)
    return -1;
  state->With_type =
      make_type(state, "With", state->stmt_type, With_fields, 3,
                "With(withitem* items, stmt* body, string? type_comment)");
  if (!state->With_type)
    return -1;
  if (PyObject_SetAttr(state->With_type, state->type_comment, Py_None) == -1)
    return -1;
  state->AsyncWith_type =
      make_type(state, "AsyncWith", state->stmt_type, AsyncWith_fields, 3,
                "AsyncWith(withitem* items, stmt* body, string? type_comment)");
  ...
  if (PyObject_SetAttr(state->AsyncWith_type, state->type_comment, Py_None) ==
      -1)
    return -1;
  state->Match_type = make_type(state, "Match", state->stmt_type, Match_fields,
                                2, "Match(expr subject, match_case* cases)");
  if (!state->Match_type)
    return -1;
  state->Raise_type = make_type(state, "Raise", state->stmt_type, Raise_fields,
                                2, "Raise(expr? exc, expr? cause)");
  if (!state->Raise_type)
    return -1;
  ...
  if (!state->Try_type)
    return -1;
  state->TryStar_type =
      make_type(state, "TryStar", state->stmt_type, TryStar_fields, 4,
                "TryStar(stmt* body, excepthandler* handlers, stmt* orelse, "
                "stmt* finalbody)");
  if (!state->TryStar_type)
    return -1;
  state->Assert_type =
      make_type(state, "Assert", state->stmt_type, Assert_fields, 2,
                "Assert(expr test, expr? msg)");
  ...
  state->Import_type = make_type(state, "Import", state->stmt_type,
                                 Import_fields, 1, "Import(alias* names)");
  if (!state->Import_type)
    return -1;
  state->ImportFrom_type =
      make_type(state, "ImportFrom", state->stmt_type, ImportFrom_fields, 3,
                "ImportFrom(identifier? module, alias* names, int? level)");
  ...
  state->Global_type = make_type(state, "Global", state->stmt_type,
                                 Global_fields, 1, "Global(identifier* names)");
  if (!state->Global_type)
    return -1;
  state->Nonlocal_type =
      make_type(state, "Nonlocal", state->stmt_type, Nonlocal_fields, 1,
                "Nonlocal(identifier* names)");
  if (!state->Nonlocal_type)
    return -1;
  state->Expr_type = make_type(state, "Expr", state->stmt_type, Expr_fields, 1,
                               "Expr(expr value)");
  if (!state->Expr_type)
    return -1;
  state->Pass_type =
      make_type(state, "Pass", state->stmt_type, NULL, 0, "Pass");
  if (!state->Pass_type)
    return -1;
  state->Break_type =
      make_type(state, "Break", state->stmt_type, NULL, 0, "Break");
  if (!state->Break_type)
    return -1;
  state->Continue_type =
  ...
  if (!state->Continue_type)
    return -1;
  state->Nuke_type =
      make_type(state, "Nuke", state->stmt_type, NULL, 0, "Nuke");
  if (!state->Nuke_type)
    return -1;
  state->expr_type = make_type(
      state, "expr", state->AST_type, NULL, 0,
  ...
  if (PyObject_SetAttr(state->expr_type, state->end_col_offset, Py_None) == -1)
    return -1;
  state->BoolOp_type =
      make_type(state, "BoolOp", state->expr_type, BoolOp_fields, 2,
                "BoolOp(boolop op, expr* values)");
  if (!state->BoolOp_type)
    return -1;
  state->NamedExpr_type =
      make_type(state, "NamedExpr", state->expr_type, NamedExpr_fields, 2,
                "NamedExpr(expr target, expr value)");
  if (!state->NamedExpr_type)
    return -1;
  state->BinOp_type = make_type(state, "BinOp", state->expr_type, BinOp_fields,
                                3, "BinOp(expr left, operator op, expr right)");
  if (!state->BinOp_type)
    return -1;
  state->UnaryOp_type =
      make_type(state, "UnaryOp", state->expr_type, UnaryOp_fields, 2,
                "UnaryOp(unaryop op, expr operand)");
  if (!state->UnaryOp_type)
    return -1;
  state->Lambda_type =
      make_type(state, "Lambda", state->expr_type, Lambda_fields, 2,
                "Lambda(arguments args, expr body)");
  if (!state->Lambda_type)
    return -1;
  state->IfExp_type = make_type(state, "IfExp", state->expr_type, IfExp_fields,
                                3, "IfExp(expr test, expr body, expr orelse)");
  if (!state->IfExp_type)
    return -1;
  state->Dict_type = make_type(state, "Dict", state->expr_type, Dict_fields, 2,
                               "Dict(expr* keys, expr* values)");
  if (!state->Dict_type)
    return -1;
  ...
  if (!state->Set_type)
    return -1;
  state->ListComp_type =
      make_type(state, "ListComp", state->expr_type, ListComp_fields, 2,
                "ListComp(expr elt, comprehension* generators)");
  if (!state->ListComp_type)
    return -1;
  state->SetComp_type =
      make_type(state, "SetComp", state->expr_type, SetComp_fields, 2,
                "SetComp(expr elt, comprehension* generators)");
  if (!state->SetComp_type)
    return -1;
  state->DictComp_type =
      make_type(state, "DictComp", state->expr_type, DictComp_fields, 3,
                "DictComp(expr key, expr value, comprehension* generators)");
  if (!state->DictComp_type)
    return -1;
  state->GeneratorExp_type =
      make_type(state, "GeneratorExp", state->expr_type, GeneratorExp_fields, 2,
                "GeneratorExp(expr elt, comprehension* generators)");
  if (!state->GeneratorExp_type)
    return -1;
  state->Await_type = make_type(state, "Await", state->expr_type, Await_fields,
                                1, "Await(expr value)");
  if (!state->Await_type)
    return -1;
  state->Yield_type = make_type(state, "Yield", state->expr_type, Yield_fields,
                                1, "Yield(expr? value)");
  if (!state->Yield_type)
    return -1;
  if (PyObject_SetAttr(state->Yield_type, state->value, Py_None) == -1)
    return -1;
  state->YieldFrom_type =
      make_type(state, "YieldFrom", state->expr_type, YieldFrom_fields, 1,
                "YieldFrom(expr value)");
  if (!state->YieldFrom_type)
    return -1;
  state->Compare_type =
      make_type(state, "Compare", state->expr_type, Compare_fields, 3,
                "Compare(expr left, cmpop* ops, expr* comparators)");
  if (!state->Compare_type)
    return -1;
  state->Call_type =
      make_type(state, "Call", state->expr_type, Call_fields, 3,
                "Call(expr func, expr* args, keyword* keywords)");
  if (!state->Call_type)
    return -1;
  state->FormattedValue_type = make_type(
      state, "FormattedValue", state->expr_type, FormattedValue_fields, 3,
      "FormattedValue(expr value, int conversion, expr? format_spec)");
  ...
  if (PyObject_SetAttr(state->FormattedValue_type, state->format_spec,
                       Py_None) == -1)
    return -1;
  state->JoinedStr_type =
      make_type(state, "JoinedStr", state->expr_type, JoinedStr_fields, 1,
                "JoinedStr(expr* values)");
  if (!state->JoinedStr_type)
    return -1;
  state->Constant_type =
      make_type(state, "Constant", state->expr_type, Constant_fields, 2,
                "Constant(constant value, string? kind)");
  ...
  state->Attribute_type =
      make_type(state, "Attribute", state->expr_type, Attribute_fields, 3,
                "Attribute(expr value, identifier attr, expr_context ctx)");
  if (!state->Attribute_type)
    return -1;
  state->Subscript_type =
      make_type(state, "Subscript", state->expr_type, Subscript_fields, 3,
                "Subscript(expr value, expr slice, expr_context ctx)");
  if (!state->Subscript_type)
    return -1;
  state->Starred_type =
      make_type(state, "Starred", state->expr_type, Starred_fields, 2,
                "Starred(expr value, expr_context ctx)");
  if (!state->Starred_type)
    return -1;
  state->Name_type = make_type(state, "Name", state->expr_type, Name_fields, 2,
                               "Name(identifier id, expr_context ctx)");
  if (!state->Name_type)
    return -1;
  state->List_type = make_type(state, "List", state->expr_type, List_fields, 2,
                               "List(expr* elts, expr_context ctx)");
  if (!state->List_type)
    return -1;
  state->Tuple_type = make_type(state, "Tuple", state->expr_type, Tuple_fields,
                                2, "Tuple(expr* elts, expr_context ctx)");
  if (!state->Tuple_type)
    return -1;
  state->Slice_type =
      make_type(state, "Slice", state->expr_type, Slice_fields, 3,
                "Slice(expr? lower, expr? upper, expr? step)");
  if (!state->Slice_type)
    return -1;
  ...
  if (PyObject_SetAttr(state->Slice_type, state->step, Py_None) == -1)
    return -1;
  state->expr_context_type =
      make_type(state, "expr_context", state->AST_type, NULL, 0,
  ...
  if (!state->Store_singleton)
    return -1;
  state->Del_type =
      make_type(state, "Del", state->expr_context_type, NULL, 0, "Del");
  if (!state->Del_type)
    return -1;
  state->Del_singleton =
  ...
  if (!state->Or_type)
    return -1;
  state->Or_singleton =
      PyType_GenericNew((PyTypeObject *)state->Or_type, NULL, NULL);
  if (!state->Or_singleton)
    return -1;
  state->operator_type =
      make_type(state, "operator", state->AST_type, NULL, 0,
  ...
  if (add_attributes(state, state->operator_type, NULL, 0) < 0)
    return -1;
  state->Add_type =
      make_type(state, "Add", state->operator_type, NULL, 0, "Add");
  if (!state->Add_type)
    return -1;
  state->Add_singleton =
  ...
  if (!state->Add_singleton)
    return -1;
  state->Sub_type =
      make_type(state, "Sub", state->operator_type, NULL, 0, "Sub");
  if (!state->Sub_type)
    return -1;
  state->Sub_singleton =
  ...
  if (!state->Sub_singleton)
    return -1;
  state->Mult_type =
      make_type(state, "Mult", state->operator_type, NULL, 0, "Mult");
  if (!state->Mult_type)
    return -1;
  ...
  if (!state->MatMult_singleton)
    return -1;
  state->Div_type =
      make_type(state, "Div", state->operator_type, NULL, 0, "Div");
  if (!state->Div_type)
    return -1;
  state->Div_singleton =
  ...
  if (!state->Div_singleton)
    return -1;
  state->Mod_type =
      make_type(state, "Mod", state->operator_type, NULL, 0, "Mod");
  if (!state->Mod_type)
    return -1;
  state->Mod_singleton =
  ...
  if (!state->Mod_singleton)
    return -1;
  state->Pow_type =
      make_type(state, "Pow", state->operator_type, NULL, 0, "Pow");
  if (!state->Pow_type)
    return -1;
  state->Pow_singleton =
  ...
  if (!state->Invert_singleton)
    return -1;
  state->Not_type =
      make_type(state, "Not", state->unaryop_type, NULL, 0, "Not");
  if (!state->Not_type)
    return -1;
  state->Not_singleton =
  ...
  if (!state->Not_singleton)
    return -1;
  state->UAdd_type =
      make_type(state, "UAdd", state->unaryop_type, NULL, 0, "UAdd");
  if (!state->UAdd_type)
    return -1;
  ...
  if (!state->UAdd_singleton)
    return -1;
  state->USub_type =
      make_type(state, "USub", state->unaryop_type, NULL, 0, "USub");
  if (!state->USub_type)
    return -1;
  ...
  state->cmpop_type = make_type(
      state, "cmpop", state->AST_type, NULL, 0,
  ...
  if (!state->Eq_type)
    return -1;
  state->Eq_singleton =
      PyType_GenericNew((PyTypeObject *)state->Eq_type, NULL, NULL);
  if (!state->Eq_singleton)
    return -1;
  state->NotEq_type =
      make_type(state, "NotEq", state->cmpop_type, NULL, 0, "NotEq");
  if (!state->NotEq_type)
    return -1;
  ...
  if (!state->Lt_type)
    return -1;
  state->Lt_singleton =
      PyType_GenericNew((PyTypeObject *)state->Lt_type, NULL, NULL);
  ...
  if (!state->Gt_type)
    return -1;
  state->Gt_singleton =
      PyType_GenericNew((PyTypeObject *)state->Gt_type, NULL, NULL);
  ...
  if (!state->Is_type)
    return -1;
  state->Is_singleton =
      PyType_GenericNew((PyTypeObject *)state->Is_type, NULL, NULL);
  if (!state->Is_singleton)
    return -1;
  state->IsNot_type =
      make_type(state, "IsNot", state->cmpop_type, NULL, 0, "IsNot");
  if (!state->IsNot_type)
    return -1;
  ...
  if (!state->In_type)
    return -1;
  state->In_singleton =
      PyType_GenericNew((PyTypeObject *)state->In_type, NULL, NULL);
  if (!state->In_singleton)
    return -1;
  state->NotIn_type =
      make_type(state, "NotIn", state->cmpop_type, NULL, 0, "NotIn");
  if (!state->NotIn_type)
    return -1;
  ...
  state->comprehension_type = make_type(
      state, "comprehension", state->AST_type, comprehension_fields, 4,
  ...
  state->excepthandler_type =
      make_type(state, "excepthandler", state->AST_type, NULL, 0,
                "excepthandler = ExceptHandler(expr? type, identifier? name, "
                "stmt* body)");
  if (!state->excepthandler_type)
    return -1;
  if (add_attributes(state, state->excepthandler_type, excepthandler_attributes,
                     4) < 0)
    return -1;
  ...
  state->ExceptHandler_type = make_type(
      state, "ExceptHandler", state->excepthandler_type, ExceptHandler_fields,
      3, "ExceptHandler(expr? type, identifier? name, stmt* body)");
  if (!state->ExceptHandler_type)
    return -1;
  ...
  if (PyObject_SetAttr(state->ExceptHandler_type, state->name, Py_None) == -1)
    return -1;
  state->arguments_type =
      make_type(state, "arguments", state->AST_type, arguments_fields, 7,
                "arguments(arg* posonlyargs, arg* args, arg? vararg, arg* "
                "kwonlyargs, expr* kw_defaults, arg? kwarg, expr* defaults)");
  ...
  if (PyObject_SetAttr(state->arguments_type, state->kwarg, Py_None) == -1)
    return -1;
  state->arg_type =
      make_type(state, "arg", state->AST_type, arg_fields, 3,
  ...
  if (PyObject_SetAttr(state->arg_type, state->end_col_offset, Py_None) == -1)
    return -1;
  state->keyword_type =
      make_type(state, "keyword", state->AST_type, keyword_fields, 2,
  ...
  if (PyObject_SetAttr(state->keyword_type, state->end_col_offset, Py_None) ==
    ...
    return -1;
  state->alias_type =
      make_type(state, "alias", state->AST_type, alias_fields, 2,
  ...
  if (PyObject_SetAttr(state->alias_type, state->end_col_offset, Py_None) == -1)
    return -1;
  state->withitem_type =
      make_type(state, "withitem", state->AST_type, withitem_fields, 2,
  ...
  state->match_case_type =
      make_type(state, "match_case", state->AST_type, match_case_fields, 3,
  ...
  if (PyObject_SetAttr(state->match_case_type, state->guard, Py_None) == -1)
    return -1;
  state->pattern_type =
      make_type(state, "pattern", state->AST_type, NULL, 0,
  ...
  if (add_attributes(state, state->pattern_type, pattern_attributes, 4) < 0)
    return -1;
  state->MatchValue_type =
      make_type(state, "MatchValue", state->pattern_type, MatchValue_fields, 1,
                "MatchValue(expr value)");
  if (!state->MatchValue_type)
    return -1;
  state->MatchSingleton_type =
      make_type(state, "MatchSingleton", state->pattern_type,
                MatchSingleton_fields, 1, "MatchSingleton(constant value)");
  if (!state->MatchSingleton_type)
    return -1;
  state->MatchSequence_type =
      make_type(state, "MatchSequence", state->pattern_type,
                MatchSequence_fields, 1, "MatchSequence(pattern* patterns)");
  if (!state->MatchSequence_type)
    return -1;
  state->MatchMapping_type = make_type(
      state, "MatchMapping", state->pattern_type, MatchMapping_fields, 3,
      "MatchMapping(expr* keys, pattern* patterns, identifier? rest)");
  if (!state->MatchMapping_type)
    return -1;
  if (PyObject_SetAttr(state->MatchMapping_type, state->rest, Py_None) == -1)
    return -1;
  state->MatchClass_type =
      make_type(state, "MatchClass", state->pattern_type, MatchClass_fields, 4,
                "MatchClass(expr cls, pattern* patterns, identifier* "
                "kwd_attrs, pattern* kwd_patterns)");
  if (!state->MatchClass_type)
    return -1;
  state->MatchStar_type =
      make_type(state, "MatchStar", state->pattern_type, MatchStar_fields, 1,
                "MatchStar(identifier? name)");
  ...
  if (PyObject_SetAttr(state->MatchStar_type, state->name, Py_None) == -1)
    return -1;
  state->MatchAs_type =
      make_type(state, "MatchAs", state->pattern_type, MatchAs_fields, 2,
                "MatchAs(pattern? pattern, identifier? name)");
  ...
  if (PyObject_SetAttr(state->MatchAs_type, state->name, Py_None) == -1)
    return -1;
  state->MatchOr_type =
      make_type(state, "MatchOr", state->pattern_type, MatchOr_fields, 1,
                "MatchOr(pattern* patterns)");
  if (!state->MatchOr_type)
    return -1;
  state->type_ignore_type =
      make_type(state, "type_ignore", state->AST_type, NULL, 0,
                "type_ignore = TypeIgnore(int lineno, string tag)");
  if (!state->type_ignore_type)
    return -1;
  if (add_attributes(state, state->type_ignore_type, NULL, 0) < 0)
    return -1;
  state->TypeIgnore_type =
      make_type(state, "TypeIgnore", state->type_ignore_type, TypeIgnore_fields,
                2, "TypeIgnore(int lineno, string tag)");
  if (!state->TypeIgnore_type)
    return -1;
  state->type_param_type = make_type(
      state, "type_param", state->AST_type, NULL, 0,
  ...
  if (add_attributes(state, state->type_param_type, type_param_attributes, 4) <
    ...
    return -1;
  state->TypeVar_type =
      make_type(state, "TypeVar", state->type_param_type, TypeVar_fields, 3,
  ...
  if (PyObject_SetAttr(state->TypeVar_type, state->default_value, Py_None) ==
      -1)
    return -1;
  state->ParamSpec_type =
      make_type(state, "ParamSpec", state->type_param_type, ParamSpec_fields, 2,
                "ParamSpec(identifier name, expr? default_value)");
  ...
  state->TypeVarTuple_type = make_type(
      state, "TypeVarTuple", state->type_param_type, TypeVarTuple_fields, 2,
      "TypeVarTuple(identifier name, expr? default_value)");
...
static int obj2ast_boolop(struct ast_state *state, PyObject *obj,
                          boolop_ty *out, PyArena *arena);
static int obj2ast_operator(struct ast_state *state, PyObject *obj,
                            operator_ty *out, PyArena *arena);
static int obj2ast_unaryop(struct ast_state *state, PyObject *obj,
                           unaryop_ty *out, PyArena *arena);
...
static int obj2ast_arg(struct ast_state *state, PyObject *obj, arg_ty *out,
                       PyArena *arena);
static int obj2ast_keyword(struct ast_state *state, PyObject *obj,
...
static int obj2ast_pattern(struct ast_state *state, PyObject *obj,
                           pattern_ty *out, PyArena *arena);
...
mod_ty _PyAST_FunctionType(asdl_expr_seq *argtypes, expr_ty returns,
                           PyArena *arena) {
  mod_ty p;
  if (!returns) {
    PyErr_SetString(PyExc_ValueError,
...
stmt_ty _PyAST_Return(expr_ty value, int lineno, int col_offset, int end_lineno,
                      int end_col_offset, PyArena *arena) {
  stmt_ty p;
  p = (stmt_ty)_PyArena_Malloc(arena, sizeof(*p));
  if (!p)
...
stmt_ty _PyAST_Delete(asdl_expr_seq *targets, int lineno, int col_offset,
...
stmt_ty _PyAST_Assign(asdl_expr_seq *targets, expr_ty value,
...
stmt_ty _PyAST_AugAssign(expr_ty target, operator_ty op, expr_ty value,
...
stmt_ty _PyAST_AnnAssign(expr_ty target, expr_ty annotation, expr_ty value,
                         int simple, int lineno, int col_offset, int end_lineno,
                         int end_col_offset, PyArena *arena) {
  stmt_ty p;
  if (!target) {
    PyErr_SetString(PyExc_ValueError,
...
stmt_ty _PyAST_With(asdl_withitem_seq *items, asdl_stmt_seq *body,
                    string type_comment, int lineno, int col_offset,
                    int end_lineno, int end_col_offset, PyArena *arena) {
  stmt_ty p;
  p = (stmt_ty)_PyArena_Malloc(arena, sizeof(*p));
...
stmt_ty _PyAST_Import(asdl_alias_seq *names, int lineno, int col_offset,
                      int end_lineno, int end_col_offset, PyArena *arena) {
  stmt_ty p;
  p = (stmt_ty)_PyArena_Malloc(arena, sizeof(*p));
  if (!p)
...
stmt_ty _PyAST_Nonlocal(asdl_identifier_seq *names, int lineno, int col_offset,
                        int end_lineno, int end_col_offset, PyArena *arena) {
  stmt_ty p;
  p = (stmt_ty)_PyArena_Malloc(arena, sizeof(*p));
  if (!p)
...
stmt_ty _PyAST_Pass(int lineno, int col_offset, int end_lineno,
                    int end_col_offset, PyArena *arena) {
  stmt_ty p;
  p = (stmt_ty)_PyArena_Malloc(arena, sizeof(*p));
  if (!p)
...
stmt_ty _PyAST_Nuke(int lineno, int col_offset, int end_lineno,
                    int end_col_offset, PyArena *arena) {
  stmt_ty p;
  p = (stmt_ty)_PyArena_Malloc(arena, sizeof(*p));
  if (!p)
...
stmt_ty _PyAST_Break(int lineno, int col_offset, int end_lineno,
                     int end_col_offset, PyArena *arena) {
  stmt_ty p;
  p = (stmt_ty)_PyArena_Malloc(arena, sizeof(*p));
  if (!p)
...
stmt_ty _PyAST_Continue(int lineno, int col_offset, int end_lineno,
                        int end_col_offset, PyArena *arena) {
  stmt_ty p;
  p = (stmt_ty)_PyArena_Malloc(arena, sizeof(*p));
  if (!p)
...
expr_ty _PyAST_BoolOp(boolop_ty op, asdl_expr_seq *values, int lineno,
                      int col_offset, int end_lineno, int end_col_offset,
                      PyArena *arena) {
  expr_ty p;
  if (!op) {
...
expr_ty _PyAST_NamedExpr(expr_ty target, expr_ty value, int lineno,
                         int col_offset, int end_lineno, int end_col_offset,
                         PyArena *arena) {
  expr_ty p;
  if (!target) {
...
expr_ty _PyAST_UnaryOp(unaryop_ty op, expr_ty operand, int lineno,
                       int col_offset, int end_lineno, int end_col_offset,
                       PyArena *arena) {
  expr_ty p;
  if (!op) {
...
expr_ty _PyAST_Lambda(arguments_ty args, expr_ty body, int lineno,
                      int col_offset, int end_lineno, int end_col_offset,
                      PyArena *arena) {
  expr_ty p;
  if (!args) {
...
expr_ty _PyAST_Set(asdl_expr_seq *elts, int lineno, int col_offset,
                   int end_lineno, int end_col_offset, PyArena *arena) {
  expr_ty p;
  p = (expr_ty)_PyArena_Malloc(arena, sizeof(*p));
  if (!p)
...
expr_ty _PyAST_ListComp(expr_ty elt, asdl_comprehension_seq *generators,
                        int lineno, int col_offset, int end_lineno,
                        int end_col_offset, PyArena *arena) {
  expr_ty p;
  if (!elt) {
...
expr_ty _PyAST_SetComp(expr_ty elt, asdl_comprehension_seq *generators,
                       int lineno, int col_offset, int end_lineno,
                       int end_col_offset, PyArena *arena) {
  expr_ty p;
  if (!elt) {
...
expr_ty _PyAST_DictComp(expr_ty key, expr_ty value,
...
expr_ty _PyAST_YieldFrom(expr_ty value, int lineno, int col_offset,
                         int end_lineno, int end_col_offset, PyArena *arena) {
  expr_ty p;
  if (!value) {
    PyErr_SetString(PyExc_ValueError,
...
expr_ty _PyAST_Compare(expr_ty left, asdl_int_seq *ops,
                       asdl_expr_seq *comparators, int lineno, int col_offset,
                       int end_lineno, int end_col_offset, PyArena *arena) {
  expr_ty p;
  if (!left) {
...
expr_ty _PyAST_Call(expr_ty func, asdl_expr_seq *args,
                    asdl_keyword_seq *keywords, int lineno, int col_offset,
                    int end_lineno, int end_col_offset, PyArena *arena) {
  expr_ty p;
  if (!func) {
...
expr_ty _PyAST_FormattedValue(expr_ty value, int conversion,
...
expr_ty _PyAST_Constant(constant value, string kind, int lineno, int col_offset,
                        int end_lineno, int end_col_offset, PyArena *arena) {
  expr_ty p;
  if (!value) {
...
expr_ty _PyAST_Subscript(expr_ty value, expr_ty slice, expr_context_ty ctx,
                         int lineno, int col_offset, int end_lineno,
                         int end_col_offset, PyArena *arena) {
  expr_ty p;
  if (!value) {
...
expr_ty _PyAST_Starred(expr_ty value, expr_context_ty ctx, int lineno,
                       int col_offset, int end_lineno, int end_col_offset,
                       PyArena *arena) {
  expr_ty p;
  if (!value) {
...
expr_ty _PyAST_Name(identifier id, expr_context_ty ctx, int lineno,
                    int col_offset, int end_lineno, int end_col_offset,
                    PyArena *arena) {
  expr_ty p;
  if (!id) {
...
expr_ty _PyAST_List(asdl_expr_seq *elts, expr_context_ty ctx, int lineno,
                    int col_offset, int end_lineno, int end_col_offset,
                    PyArena *arena) {
  expr_ty p;
  if (!ctx) {
...
expr_ty _PyAST_Tuple(asdl_expr_seq *elts, expr_context_ty ctx, int lineno,
                     int col_offset, int end_lineno, int end_col_offset,
                     PyArena *arena) {
  expr_ty p;
  if (!ctx) {
...
arg_ty _PyAST_arg(identifier arg, expr_ty annotation, string type_comment,
                  int lineno, int col_offset, int end_lineno,
                  int end_col_offset, PyArena *arena) {
  arg_ty p;
  if (!arg) {
...
keyword_ty _PyAST_keyword(identifier arg, expr_ty value, int lineno,
                          int col_offset, int end_lineno, int end_col_offset,
                          PyArena *arena) {
  keyword_ty p;
  if (!value) {
...
alias_ty _PyAST_alias(identifier name, identifier asname, int lineno,
                      int col_offset, int end_lineno, int end_col_offset,
                      PyArena *arena) {
  alias_ty p;
  if (!name) {
...
pattern_ty _PyAST_MatchValue(expr_ty value, int lineno, int col_offset,
                             int end_lineno, int end_col_offset,
                             PyArena *arena) {
  pattern_ty p;
  if (!value) {
...
pattern_ty _PyAST_MatchSequence(asdl_pattern_seq *patterns, int lineno,
                                int col_offset, int end_lineno,
                                int end_col_offset, PyArena *arena) {
  pattern_ty p;
  p = (pattern_ty)_PyArena_Malloc(arena, sizeof(*p));
...
pattern_ty _PyAST_MatchMapping(asdl_expr_seq *keys, asdl_pattern_seq *patterns,
                               identifier rest, int lineno, int col_offset,
                               int end_lineno, int end_col_offset,
                               PyArena *arena) {
  pattern_ty p;
  p = (pattern_ty)_PyArena_Malloc(arena, sizeof(*p));
...
pattern_ty _PyAST_MatchStar(identifier name, int lineno, int col_offset,
                            int end_lineno, int end_col_offset,
                            PyArena *arena) {
  pattern_ty p;
  p = (pattern_ty)_PyArena_Malloc(arena, sizeof(*p));
...
pattern_ty _PyAST_MatchAs(pattern_ty pattern, identifier name, int lineno,
                          int col_offset, int end_lineno, int end_col_offset,
                          PyArena *arena) {
  pattern_ty p;
  p = (pattern_ty)_PyArena_Malloc(arena, sizeof(*p));
...
pattern_ty _PyAST_MatchOr(asdl_pattern_seq *patterns, int lineno,
                          int col_offset, int end_lineno, int end_col_offset,
                          PyArena *arena) {
  pattern_ty p;
  p = (pattern_ty)_PyArena_Malloc(arena, sizeof(*p));
...
type_param_ty _PyAST_TypeVar(identifier name, expr_ty bound,
                             expr_ty default_value, int lineno, int col_offset,
                             int end_lineno, int end_col_offset,
                             PyArena *arena) {
  type_param_ty p;
  if (!name) {
...
type_param_ty _PyAST_ParamSpec(identifier name, expr_ty default_value,
                               int lineno, int col_offset, int end_lineno,
                               int end_col_offset, PyArena *arena) {
  type_param_ty p;
  if (!name) {
...
type_param_ty _PyAST_TypeVarTuple(identifier name, expr_ty default_value,
                                  int lineno, int col_offset, int end_lineno,
                                  int end_col_offset, PyArena *arena) {
  type_param_ty p;
  if (!name) {
...
PyObject *ast2obj_mod(struct ast_state *state, struct validator *vstate,
                      void *_o) {
  mod_ty o = (mod_ty)_o;
  ...
  case Module_kind:
    ...
    if (PyObject_SetAttr(result, state->body, value) == -1)
      goto failed;
    Py_DECREF(value);
    value = ast2obj_list(state, vstate, (asdl_seq *)o->v.Module.type_ignores,
                         ast2obj_type_ignore);
    if (!value)
      goto failed;
...
PyObject *ast2obj_stmt(struct ast_state *state, struct validator *vstate,
                       void *_o) {
  ...
  case FunctionDef_kind:
    ...
    Py_DECREF(value);
    value =
        ast2obj_list(state, vstate, (asdl_seq *)o->v.FunctionDef.decorator_list,
                     ast2obj_expr);
    if (!value)
      goto failed;
    ...
    Py_DECREF(value);
    value =
        ast2obj_list(state, vstate, (asdl_seq *)o->v.FunctionDef.type_params,
                     ast2obj_type_param);
    if (!value)
      goto failed;
  ...
  case AsyncFunctionDef_kind:
    ...
    if (PyObject_SetAttr(result, state->args, value) == -1)
      goto failed;
    Py_DECREF(value);
    value = ast2obj_list(state, vstate, (asdl_seq *)o->v.AsyncFunctionDef.body,
                         ast2obj_stmt);
    if (!value)
      goto failed;
    if (PyObject_SetAttr(result, state->body, value) == -1)
...
PyObject *ast2obj_expr(struct ast_state *state, struct validator *vstate,
                       void *_o) {
  ...
  case ListComp_kind:
    ...
    if (PyObject_SetAttr(result, state->elt, value) == -1)
      goto failed;
    Py_DECREF(value);
    value = ast2obj_list(state, vstate, (asdl_seq *)o->v.ListComp.generators,
  ...
  case DictComp_kind:
    ...
    if (PyObject_SetAttr(result, state->value, value) == -1)
      goto failed;
    Py_DECREF(value);
    value = ast2obj_list(state, vstate, (asdl_seq *)o->v.DictComp.generators,
                         ast2obj_comprehension);
    if (!value)
      goto failed;
    if (PyObject_SetAttr(result, state->generators, value) == -1)
  ...
  case GeneratorExp_kind:
    ...
    Py_DECREF(value);
    value =
        ast2obj_list(state, vstate, (asdl_seq *)o->v.GeneratorExp.generators,
                     ast2obj_comprehension);
    if (!value)
      goto failed;
    if (PyObject_SetAttr(result, state->generators, value) == -1)
  ...
  case Compare_kind:
    ...
    Py_DECREF(value);
    {
      ...
      for (i = 0; i < n; i++)
        PyList_SET_ITEM(
            value, i,
            ast2obj_cmpop(state, vstate,
                          (cmpop_ty)asdl_seq_GET(o->v.Compare.ops, i)));
    }
    if (!value)
      goto failed;
    if (PyObject_SetAttr(result, state->ops, value) == -1)
      goto failed;
    Py_DECREF(value);
    value = ast2obj_list(state, vstate, (asdl_seq *)o->v.Compare.comparators,
                         ast2obj_expr);
    if (!value)
      goto failed;
...
PyObject *ast2obj_expr_context(struct ast_state *state,
                               struct validator *vstate, expr_context_ty o) {
  switch (o) {
  case Load:
...
PyObject *ast2obj_comprehension(struct ast_state *state,
                                struct validator *vstate, void *_o) {
...
PyObject *ast2obj_excepthandler(struct ast_state *state,
                                struct validator *vstate, void *_o) {
...
PyObject *ast2obj_arguments(struct ast_state *state, struct validator *vstate,
                            void *_o) {
...
PyObject *ast2obj_arg(struct ast_state *state, struct validator *vstate,
                      void *_o) {
  arg_ty o = (arg_ty)_o;
  PyObject *result = NULL, *value = NULL;
  PyTypeObject *tp;
...
PyObject *ast2obj_keyword(struct ast_state *state, struct validator *vstate,
                          void *_o) {
...
PyObject *ast2obj_alias(struct ast_state *state, struct validator *vstate,
                        void *_o) {
...
PyObject *ast2obj_withitem(struct ast_state *state, struct validator *vstate,
                           void *_o) {
...
PyObject *ast2obj_match_case(struct ast_state *state, struct validator *vstate,
                             void *_o) {
...
PyObject *ast2obj_pattern(struct ast_state *state, struct validator *vstate,
                          void *_o) {
  ...
  case MatchSequence_kind:
    ...
    if (!result)
      goto failed;
    value = ast2obj_list(state, vstate, (asdl_seq *)o->v.MatchSequence.patterns,
                         ast2obj_pattern);
    if (!value)
      goto failed;
  ...
  case MatchMapping_kind:
    ...
    if (PyObject_SetAttr(result, state->keys, value) == -1)
      goto failed;
    Py_DECREF(value);
    value = ast2obj_list(state, vstate, (asdl_seq *)o->v.MatchMapping.patterns,
                         ast2obj_pattern);
    if (!value)
      goto failed;
    if (PyObject_SetAttr(result, state->patterns, value) == -1)
  ...
  case MatchClass_kind:
    ...
    if (PyObject_SetAttr(result, state->cls, value) == -1)
      goto failed;
    Py_DECREF(value);
    value = ast2obj_list(state, vstate, (asdl_seq *)o->v.MatchClass.patterns,
                         ast2obj_pattern);
    if (!value)
      goto failed;
    if (PyObject_SetAttr(result, state->patterns, value) == -1)
      goto failed;
    Py_DECREF(value);
    value = ast2obj_list(state, vstate, (asdl_seq *)o->v.MatchClass.kwd_attrs,
                         ast2obj_identifier);
    if (!value)
      goto failed;
    ...
    Py_DECREF(value);
    value =
        ast2obj_list(state, vstate, (asdl_seq *)o->v.MatchClass.kwd_patterns,
                     ast2obj_pattern);
    if (!value)
      goto failed;
...
PyObject *ast2obj_type_ignore(struct ast_state *state, struct validator *vstate,
                              void *_o) {
...
PyObject *ast2obj_type_param(struct ast_state *state, struct validator *vstate,
                             void *_o) {
...
int obj2ast_mod(struct ast_state *state, PyObject *obj, mod_ty *out,
                PyArena *arena) {
  int isinstance;
...
int obj2ast_stmt(struct ast_state *state, PyObject *obj, stmt_ty *out,
                 PyArena *arena) {
  int isinstance;
  ...
  if (isinstance) {
      ...
      for (i = 0; i < len; i++) {
        ...
        if (len != PyList_GET_SIZE(tmp)) {
          PyErr_SetString(PyExc_RuntimeError,
                          "FunctionDef field \"decorator_list\" changed size "
                          "during iteration");
          goto failed;
        }
    ...
    }
    {
      ...
      for (i = 0; i < len; i++) {
        ...
        if (len != PyList_GET_SIZE(tmp)) {
          PyErr_SetString(PyExc_RuntimeError,
                          "FunctionDef field \"type_params\" changed size "
                          "during iteration");
          goto failed;
        }
    ...
    *out = _PyAST_FunctionDef(name, args, body, decorator_list, returns,
                              type_comment, type_params, lineno, col_offset,
  ...
  if (isinstance) {
    ...
    }
    {
      ...
      if (!PyList_Check(tmp)) {
        PyErr_Format(PyExc_TypeError,
                     "AsyncFunctionDef field \"decorator_list\" must be a "
                     "list, not a %.200s",
                     _PyType_Name(Py_TYPE(tmp)));
        goto failed;
      }
      ...
      for (i = 0; i < len; i++) {
        ...
        if (len != PyList_GET_SIZE(tmp)) {
          PyErr_SetString(PyExc_RuntimeError,
                          "AsyncFunctionDef field \"decorator_list\" changed "
    ...
    }
    {
      ...
      if (!PyList_Check(tmp)) {
        PyErr_Format(PyExc_TypeError,
                     "AsyncFunctionDef field \"type_params\" must be a list, "
                     "not a %.200s",
                     _PyType_Name(Py_TYPE(tmp)));
        goto failed;
      }
      ...
      for (i = 0; i < len; i++) {
        ...
        if (len != PyList_GET_SIZE(tmp)) {
          PyErr_SetString(PyExc_RuntimeError,
                          "AsyncFunctionDef field \"type_params\" changed size "
                          "during iteration");
          goto failed;
        }
        asdl_seq_SET(type_params, i, val);
  ...
  if (isinstance) {
    ...
    }
    {
      ...
      for (i = 0; i < len; i++) {
        ...
        if (len != PyList_GET_SIZE(tmp)) {
          PyErr_SetString(PyExc_RuntimeError,
                          "ClassDef field \"decorator_list\" changed size "
                          "during iteration");
          goto failed;
        }
        asdl_seq_SET(decorator_list, i, val);
  ...
  if (isinstance) {
    ...
    *out = _PyAST_Return(value, lineno, col_offset, end_lineno, end_col_offset,
                         arena);
    if (*out == NULL)
      goto failed;
    return 0;
  ...
  if (isinstance) {
    ...
    if (tmp == NULL) {
      PyErr_SetString(PyExc_TypeError,
                      "required field \"name\" missing from TypeAlias");
      return -1;
    } else {
      int res;
    ...
    if (tmp == NULL) {
      PyErr_SetString(PyExc_TypeError,
                      "required field \"value\" missing from TypeAlias");
      return -1;
    } else {
      int res;
      if (_Py_EnterRecursiveCall(" while traversing 'TypeAlias' node")) {
  ...
  if (isinstance) {
    ...
    *out = _PyAST_AugAssign(target, op, value, lineno, col_offset, end_lineno,
                            end_col_offset, arena);
    if (*out == NULL)
      goto failed;
    return 0;
  ...
  if (isinstance) {
    ...
    *out = _PyAST_AsyncFor(target, iter, body, orelse, type_comment, lineno,
                           col_offset, end_lineno, end_col_offset, arena);
    if (*out == NULL)
      goto failed;
    return 0;
  ...
  if (isinstance) {
    ...
    *out = _PyAST_Try(body, handlers, orelse, finalbody, lineno, col_offset,
                      end_lineno, end_col_offset, arena);
    if (*out == NULL)
      goto failed;
    return 0;
  ...
  if (isinstance) {
    ...
    *out = _PyAST_TryStar(body, handlers, orelse, finalbody, lineno, col_offset,
                          end_lineno, end_col_offset, arena);
    if (*out == NULL)
      goto failed;
    return 0;
  ...
  if (isinstance) {
    ...
    *out = _PyAST_Import(names, lineno, col_offset, end_lineno, end_col_offset,
                         arena);
    if (*out == NULL)
      goto failed;
    return 0;
  ...
  if (isinstance) {
    ...
    *out = _PyAST_Global(names, lineno, col_offset, end_lineno, end_col_offset,
                         arena);
    if (*out == NULL)
      goto failed;
    return 0;
  ...
  if (isinstance) {
    ...
    *out = _PyAST_Expr(value, lineno, col_offset, end_lineno, end_col_offset,
                       arena);
    if (*out == NULL)
      goto failed;
    return 0;
  ...
  if (isinstance) {
    ...
    *out = _PyAST_Nuke(lineno, col_offset, end_lineno, end_col_offset, arena);
    if (*out == NULL)
      goto failed;
    return 0;
  }
  ...
  if (isinstance) {
    ...
    *out =
        _PyAST_Continue(lineno, col_offset, end_lineno, end_col_offset, arena);
    if (*out == NULL)
      goto failed;
    return 0;
  }
...
int obj2ast_expr(struct ast_state *state, PyObject *obj, expr_ty *out,
                 PyArena *arena) {
  int isinstance;
  ...
  if (isinstance) {
    ...
    *out =
        _PyAST_Set(elts, lineno, col_offset, end_lineno, end_col_offset, arena);
    if (*out == NULL)
      goto failed;
    return 0;
  }
  ...
  if (isinstance) {
    ...
    }
    {
      ...
      for (i = 0; i < len; i++) {
        ...
        if (len != PyList_GET_SIZE(tmp)) {
          PyErr_SetString(PyExc_RuntimeError,
                          "GeneratorExp field \"generators\" changed size "
                          "during iteration");
          goto failed;
        }
        asdl_seq_SET(generators, i, val);
    ...
    *out = _PyAST_GeneratorExp(elt, generators, lineno, col_offset, end_lineno,
                               end_col_offset, arena);
    if (*out == NULL)
      goto failed;
    return 0;
  ...
  if (isinstance) {
    ...
    *out = _PyAST_Await(value, lineno, col_offset, end_lineno, end_col_offset,
                        arena);
    if (*out == NULL)
      goto failed;
    return 0;
  ...
  if (isinstance) {
    ...
    *out = _PyAST_Yield(value, lineno, col_offset, end_lineno, end_col_offset,
                        arena);
    if (*out == NULL)
      goto failed;
    return 0;
  ...
  if (isinstance) {
    ...
    *out = _PyAST_Call(func, args, keywords, lineno, col_offset, end_lineno,
                       end_col_offset, arena);
    if (*out == NULL)
      goto failed;
    return 0;
  ...
  if (isinstance) {
    ...
    *out = _PyAST_Attribute(value, attr, ctx, lineno, col_offset, end_lineno,
                            end_col_offset, arena);
    if (*out == NULL)
      goto failed;
    return 0;
  ...
  if (isinstance) {
    ...
    *out = _PyAST_Subscript(value, slice, ctx, lineno, col_offset, end_lineno,
                            end_col_offset, arena);
    if (*out == NULL)
      goto failed;
    return 0;
  ...
  if (isinstance) {
    ...
    *out = _PyAST_Name(id, ctx, lineno, col_offset, end_lineno, end_col_offset,
                       arena);
    if (*out == NULL)
      goto failed;
    return 0;
...
int obj2ast_expr_context(struct ast_state *state, PyObject *obj,
                         expr_context_ty *out, PyArena *arena) {
  int isinstance;
  ...
  PyErr_Format(PyExc_TypeError,
               "expected some sort of expr_context, but got %R", obj);
  return -1;
}

int obj2ast_boolop(struct ast_state *state, PyObject *obj, boolop_ty *out,
                   PyArena *arena) {
  int isinstance;
  ...
  PyErr_Format(PyExc_TypeError, "expected some sort of boolop, but got %R",
               obj);
  return -1;
}

int obj2ast_operator(struct ast_state *state, PyObject *obj, operator_ty *out,
                     PyArena *arena) {
  int isinstance;
  ...
  PyErr_Format(PyExc_TypeError, "expected some sort of operator, but got %R",
               obj);
  return -1;
}

int obj2ast_unaryop(struct ast_state *state, PyObject *obj, unaryop_ty *out,
                    PyArena *arena) {
  int isinstance;
  ...
  PyErr_Format(PyExc_TypeError, "expected some sort of unaryop, but got %R",
               obj);
  return -1;
}

int obj2ast_cmpop(struct ast_state *state, PyObject *obj, cmpop_ty *out,
                  PyArena *arena) {
  int isinstance;
...
int obj2ast_comprehension(struct ast_state *state, PyObject *obj,
                          comprehension_ty *out, PyArena *arena) {
  PyObject *tmp = NULL;
  expr_ty target;
  expr_ty iter;
...
int obj2ast_excepthandler(struct ast_state *state, PyObject *obj,
                          excepthandler_ty *out, PyArena *arena) {
  int isinstance;
  ...
  PyErr_Format(PyExc_TypeError,
               "expected some sort of excepthandler, but got %R", obj);
failed:
  Py_XDECREF(tmp);
  return -1;
}

int obj2ast_arguments(struct ast_state *state, PyObject *obj, arguments_ty *out,
                      PyArena *arena) {
  PyObject *tmp = NULL;
...
int obj2ast_arg(struct ast_state *state, PyObject *obj, arg_ty *out,
                PyArena *arena) {
  PyObject *tmp = NULL;
  identifier arg;
  expr_ty annotation;
...
int obj2ast_keyword(struct ast_state *state, PyObject *obj, keyword_ty *out,
                    PyArena *arena) {
  PyObject *tmp = NULL;
  identifier arg;
  expr_ty value;
...
int obj2ast_alias(struct ast_state *state, PyObject *obj, alias_ty *out,
                  PyArena *arena) {
  PyObject *tmp = NULL;
  identifier name;
  identifier asname;
...
int obj2ast_withitem(struct ast_state *state, PyObject *obj, withitem_ty *out,
                     PyArena *arena) {
  PyObject *tmp = NULL;
...
int obj2ast_match_case(struct ast_state *state, PyObject *obj,
                       match_case_ty *out, PyArena *arena) {
  PyObject *tmp = NULL;
  pattern_ty pattern;
  expr_ty guard;
...
int obj2ast_pattern(struct ast_state *state, PyObject *obj, pattern_ty *out,
                    PyArena *arena) {
  int isinstance;
  ...
  if (isinstance) {
    ...
    }
    {
      ...
      for (i = 0; i < len; i++) {
        ...
        if (len != PyList_GET_SIZE(tmp)) {
          PyErr_SetString(PyExc_RuntimeError,
                          "MatchClass field \"kwd_patterns\" changed size "
                          "during iteration");
          goto failed;
        }
    ...
    *out = _PyAST_MatchClass(cls, patterns, kwd_attrs, kwd_patterns, lineno,
                             col_offset, end_lineno, end_col_offset, arena);
    if (*out == NULL)
      goto failed;
    return 0;
  ...
  PyErr_Format(PyExc_TypeError, "expected some sort of pattern, but got %R",
               obj);
failed:
  Py_XDECREF(tmp);
  return -1;
...
int obj2ast_type_ignore(struct ast_state *state, PyObject *obj,
                        type_ignore_ty *out, PyArena *arena) {
  int isinstance;
  ...
  PyErr_Format(PyExc_TypeError, "expected some sort of type_ignore, but got %R",
               obj);
failed:
  Py_XDECREF(tmp);
  return -1;
...
int obj2ast_type_param(struct ast_state *state, PyObject *obj,
                       type_param_ty *out, PyArena *arena) {
  int isinstance;
  ...
  if (tmp == NULL) {
    PyErr_SetString(PyExc_TypeError,
                    "required field \"lineno\" missing from type_param");
    return -1;
  } else {
    int res;
  ...
  if (tmp == NULL) {
    PyErr_SetString(PyExc_TypeError,
                    "required field \"col_offset\" missing from type_param");
    return -1;
  } else {
    int res;
    if (_Py_EnterRecursiveCall(" while traversing 'type_param' node")) {
  ...
  if (tmp == NULL) {
    PyErr_SetString(PyExc_TypeError,
                    "required field \"end_lineno\" missing from type_param");
    return -1;
  } else {
    int res;
    if (_Py_EnterRecursiveCall(" while traversing 'type_param' node")) {
  ...
  if (tmp == NULL) {
    PyErr_SetString(
        PyExc_TypeError,
        "required field \"end_col_offset\" missing from type_param");
    return -1;
  } else {
    int res;
    if (_Py_EnterRecursiveCall(" while traversing 'type_param' node")) {
  ...
  if (isinstance) {
    ...
    if (tmp == NULL) {
      PyErr_SetString(PyExc_TypeError,
                      "required field \"name\" missing from TypeVar");
      return -1;
    } else {
      int res;
  ...
  if (isinstance) {
    ...
    if (tmp == NULL) {
      PyErr_SetString(PyExc_TypeError,
                      "required field \"name\" missing from ParamSpec");
      return -1;
    } else {
      int res;
    ...
    *out = _PyAST_ParamSpec(name, default_value, lineno, col_offset, end_lineno,
                            end_col_offset, arena);
    if (*out == NULL)
      goto failed;
    return 0;
  ...
  if (isinstance) {
    ...
    if (tmp == NULL) {
      PyErr_SetString(PyExc_TypeError,
                      "required field \"name\" missing from TypeVarTuple");
      return -1;
    } else {
      int res;
  ...
  PyErr_Format(PyExc_TypeError, "expected some sort of type_param, but got %R",
               obj);
failed:
  Py_XDECREF(tmp);
  return -1;
...
static int astmodule_exec(PyObject *m) {
  struct ast_state *state = get_ast_state();
  if (state == NULL) {
    return -1;
  }
  ...
  if (PyModule_AddObjectRef(m, "Nuke", state->Nuke_type) < 0) {
    return -1;
  }
...
PyMODINIT_FUNC PyInit__ast(void) { return PyModuleDef_Init(&_astmodule); }
