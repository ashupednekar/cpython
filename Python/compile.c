 ...
 *   4. Generate a control flow graph and run optimizations on it.  See
 * flowgraph.c.
 *   5. Assemble the basic blocks into final code.  See optimize_and_assemble()
 * in this file, and assembler.c.
 *
...
static inline bool location_is_after(location loc1, location loc2) {
...
static cfg_builder *instr_sequence_to_cfg(instr_sequence *seq) {
...
static int codegen_addop_i(instr_sequence *seq, int opcode, Py_ssize_t oparg,
                           location loc);
...
static int compiler_nameop(struct compiler *, location, identifier,
                           expr_context_ty);
...
static int compiler_call_simple_kw_helper(struct compiler *c, location loc,
                                          asdl_keyword_seq *keywords,
                                          Py_ssize_t nkwelts);
static int compiler_call_helper(struct compiler *c, location loc, int n,
                                asdl_expr_seq *args,
                                asdl_keyword_seq *keywords);
...
compiler_sync_comprehension_generator(struct compiler *c, location loc,
                                      ...
                                      int gen_index, int depth, expr_ty elt,
                                      expr_ty val, int type, int iter_on_stack);

static int compiler_async_comprehension_generator(
    struct compiler *c, location loc, asdl_comprehension_seq *generators,
...
static int compiler_setup(struct compiler *c, mod_ty mod, PyObject *filename,
  ...
  c->c_flags = *flags;
  c->c_optimize =
      (optimize == -1) ? _Py_GetConfig()->optimization_level : optimize;
  c->c_nestlevel = 0;
...
static struct compiler *new_compiler(mod_ty mod, PyObject *filename,
...
static PyObject *list2dict(PyObject *list) {
  Py_ssize_t i, n;
  PyObject *v, *k;
...
static PyObject *dictbytype(PyObject *src, int scope_type, int flag,
                            Py_ssize_t offset) {
...
static struct compiler_unit *get_class_compiler_unit(struct compiler *c) {
...
static int compiler_set_qualname(struct compiler *c) {
...
/* Return the stack effect of opcode with argument oparg.
   ...
   * -1 -- maximal
 */
static int stack_effect(int opcode, int oparg, int jump) {
...
int _PyCompile_OpcodeIsValid(int opcode) { return IS_VALID_OPCODE(opcode); }

int _PyCompile_OpcodeHasArg(int opcode) { return OPCODE_HAS_ARG(opcode); }

int _PyCompile_OpcodeHasConst(int opcode) { return OPCODE_HAS_CONST(opcode); }

int _PyCompile_OpcodeHasName(int opcode) { return OPCODE_HAS_NAME(opcode); }

int _PyCompile_OpcodeHasJump(int opcode) { return OPCODE_HAS_JUMP(opcode); }

int _PyCompile_OpcodeHasFree(int opcode) { return OPCODE_HAS_FREE(opcode); }

int _PyCompile_OpcodeHasLocal(int opcode) { return OPCODE_HAS_LOCAL(opcode); }

int _PyCompile_OpcodeHasExc(int opcode) { return IS_BLOCK_PUSH_OPCODE(opcode); }

static int codegen_addop_noarg(instr_sequence *seq, int opcode, location loc) {
  assert(!OPCODE_HAS_ARG(opcode));
  assert(!IS_ASSEMBLER_OPCODE(opcode));
  return _PyInstructionSequence_Addop(seq, opcode, 0, loc);
}

static Py_ssize_t dict_add_o(PyObject *dict, PyObject *o) {
  PyObject *v;
  Py_ssize_t arg;
...
static Py_ssize_t compiler_add_const(PyObject *const_cache,
                                     struct compiler_unit *u, PyObject *o) {
  assert(PyDict_CheckExact(const_cache));
...
static int compiler_addop_load_const(PyObject *const_cache,
                                     struct compiler_unit *u, location loc,
                                     PyObject *o) {
...
static int compiler_addop_o(struct compiler_unit *u, location loc, int opcode,
...
static int compiler_addop_name(struct compiler_unit *u, location loc,
...
static int codegen_addop_j(instr_sequence *seq, location loc, int opcode,
...
#define ADDOP_IN_SCOPE(C, LOC, OP)                                             \
  RETURN_IF_ERROR_IN_SCOPE(                                                    \
      (C), codegen_addop_noarg(INSTR_SEQUENCE(C), (OP), (LOC)))

#define ADDOP_LOAD_CONST(C, LOC, O)                                            \
  RETURN_IF_ERROR(                                                             \
      compiler_addop_load_const((C)->c_const_cache, (C)->u, (LOC), (O)))
...
#define ADDOP_LOAD_CONST_NEW(C, LOC, O)                                        \
  {                                                                            \
    ...
    if (compiler_addop_load_const((C)->c_const_cache, (C)->u, (LOC),           \
                                  __new_const) < 0) {                          \
...
#define ADDOP_N(C, LOC, OP, O, TYPE)                                           \
  {                                                                            \
    ...
    if (compiler_addop_o((C)->u, (LOC), (OP), (C)->u->u_metadata.u_##TYPE,     \
                         (O)) < 0) {                                           \
      Py_DECREF((O));                                                          \
      return ERROR;                                                            \
    }                                                                          \
...
#define ADDOP_NAME(C, LOC, OP, O, TYPE)                                        \
  RETURN_IF_ERROR(compiler_addop_name((C)->u, (LOC), (OP),                     \
                                      (C)->u->u_metadata.u_##TYPE, (O)))
...
#define ADDOP_YIELD(C, LOC) RETURN_IF_ERROR(addop_yield((C), (LOC)))

/* VISIT and VISIT_SEQ takes an ASDL type as their second argument.  They use
   the ASDL name to synthesize the name of the C type and the visit function.
*/

#define VISIT(C, TYPE, V) RETURN_IF_ERROR(compiler_visit_##TYPE((C), (V)));
...
static int compiler_enter_scope(struct compiler *c, identifier name,
  ...
  u->u_metadata.u_freevars =
      dictbytype(u->u_ste->ste_symbols, FREE, DEF_FREE_CLASS,
...
static bool find_ann(asdl_stmt_seq *stmts) {
  int i, j, res = 0;
  stmt_ty st;
  ...
  for (i = 0; i < asdl_seq_LEN(stmts); i++) {
    ...
    case AnnAssign_kind:
      return true;
    case For_kind:
      res = find_ann(st->v.For.body) || find_ann(st->v.For.orelse);
      break;
    case AsyncFor_kind:
      res = find_ann(st->v.AsyncFor.body) || find_ann(st->v.AsyncFor.orelse);
      break;
    case While_kind:
      res = find_ann(st->v.While.body) || find_ann(st->v.While.orelse);
      break;
    case If_kind:
      res = find_ann(st->v.If.body) || find_ann(st->v.If.orelse);
      break;
    case With_kind:
    ...
    case Try_kind:
      for (j = 0; j < asdl_seq_LEN(st->v.Try.handlers); j++) {
        excepthandler_ty handler =
            (excepthandler_ty)asdl_seq_GET(st->v.Try.handlers, j);
        if (find_ann(handler->v.ExceptHandler.body)) {
          return true;
        }
      }
      res = find_ann(st->v.Try.body) || find_ann(st->v.Try.finalbody) ||
            find_ann(st->v.Try.orelse);
      break;
    case TryStar_kind:
      for (j = 0; j < asdl_seq_LEN(st->v.TryStar.handlers); j++) {
        excepthandler_ty handler =
            (excepthandler_ty)asdl_seq_GET(st->v.TryStar.handlers, j);
        if (find_ann(handler->v.ExceptHandler.body)) {
          return true;
        }
      }
      res = find_ann(st->v.TryStar.body) || find_ann(st->v.TryStar.finalbody) ||
            find_ann(st->v.TryStar.orelse);
      break;
    case Match_kind:
      for (j = 0; j < asdl_seq_LEN(st->v.Match.cases); j++) {
        match_case_ty match_case =
            (match_case_ty)asdl_seq_GET(st->v.Match.cases, j);
...
static int compiler_call_exit_with_nones(struct compiler *c, location loc) {
...
static int compiler_add_yield_from(struct compiler *c, location loc,
                                   int await) {
...
static int compiler_pop_except_and_reraise(struct compiler *c, location loc) {
  /* Stack contents
 ...
 * be popped.
 */
static int compiler_unwind_fblock(struct compiler *c, location *ploc,
 ...
 * encountered. */
static int compiler_unwind_fblock_stack(struct compiler *c, location *ploc,
                                        int preserve_tos,
                                        struct fblockinfo **loop) {
  if (c->u->u_nfblocks == 0) {
    return SUCCESS;
  }
  ...
  if (loop != NULL &&
      (top->fb_type == WHILE_LOOP || top->fb_type == FOR_LOOP)) {
    *loop = top;
    return SUCCESS;
  }
...
static int compiler_body(struct compiler *c, location loc,
                         asdl_stmt_seq *stmts) {
...
static int compiler_codegen(struct compiler *c, mod_ty mod) {
  ...
  case Expression_kind:
    ...
    break;
  default:
    PyErr_Format(PyExc_SystemError, "module kind %d should not be possible",
                 mod->kind);
    return ERROR;
  }
  return SUCCESS;
...
static int compiler_enter_anonymous_scope(struct compiler *c, mod_ty mod) {
  _Py_DECLARE_STR(anon_module, "<module>");
  RETURN_IF_ERROR(compiler_enter_scope(c, &_Py_STR(anon_module),
                                       COMPILER_SCOPE_MODULE, mod, 1));
  return SUCCESS;
}
...
static int get_ref_type(struct compiler *c, PyObject *name) {
  int scope;
  ...
  if (scope == 0) {
    PyErr_Format(PyExc_SystemError,
                 ...
                 name, c->u->u_metadata.u_name, c->u->u_ste->ste_id,
                 c->u->u_ste->ste_symbols, c->u->u_metadata.u_varnames,
...
static int compiler_lookup_arg(PyObject *dict, PyObject *name) {
      ...
      */
      ...
      if (arg == -1) {
        ...
        PyErr_Format(
            ...
            name, reftype, c->u->u_metadata.u_name, co->co_name, freevars);
        Py_DECREF(freevars);
        return ERROR;
      }
...
static int compiler_decorators(struct compiler *c, asdl_expr_seq *decos) {
  if (!decos) {
    return SUCCESS;
  }
...
static int compiler_apply_decorators(struct compiler *c, asdl_expr_seq *decos) {
  if (!decos) {
    return SUCCESS;
  }
...
static int compiler_visit_kwonlydefaults(struct compiler *c, location loc,
...
static int compiler_visit_argannotation(struct compiler *c, identifier id,
                                        expr_ty annotation,
...
static int compiler_visit_argannotations(struct compiler *c, asdl_arg_seq *args,
                                         Py_ssize_t *annotations_len,
                                         location loc) {
  int i;
  for (i = 0; i < asdl_seq_LEN(args); i++) {
    arg_ty arg = (arg_ty)asdl_seq_GET(args, i);
    RETURN_IF_ERROR(compiler_visit_argannotation(c, arg->arg, arg->annotation,
                                                 annotations_len, loc));
  }
  return SUCCESS;
}

static int compiler_visit_annotations(struct compiler *c, location loc,
                                      arguments_ty args, expr_ty returns) {
  /* Push arg annotation names and values.
     The expressions are evaluated out-of-order wrt the source code.

     Return -1 on error, 0 if no annotations pushed, 1 if a annotations is
     pushed.
     */
  ...
  RETURN_IF_ERROR(compiler_visit_argannotations(c, args->posonlyargs,
                                                &annotations_len, loc));

  if (args->vararg && args->vararg->annotation) {
    RETURN_IF_ERROR(compiler_visit_argannotation(
        c, args->vararg->arg, args->vararg->annotation, &annotations_len, loc));
  }

  RETURN_IF_ERROR(compiler_visit_argannotations(c, args->kwonlyargs,
                                                &annotations_len, loc));
  ...
  RETURN_IF_ERROR(compiler_visit_argannotation(c, &_Py_ID(return), returns,
                                               &annotations_len, loc));
...
static int compiler_visit_defaults(struct compiler *c, arguments_ty args,
                                   location loc) {
...
static Py_ssize_t compiler_default_arguments(struct compiler *c, location loc,
                                             arguments_ty args) {
...
static bool forbidden_name(struct compiler *c, location loc, identifier name,
                           expr_context_ty ctx) {
...
static int compiler_check_debug_one_arg(struct compiler *c, arg_ty arg) {
  if (arg != NULL) {
...
static int compiler_check_debug_args_seq(struct compiler *c,
                                         asdl_arg_seq *args) {
  if (args != NULL) {
    for (Py_ssize_t i = 0, n = asdl_seq_LEN(args); i < n; i++) {
      RETURN_IF_ERROR(compiler_check_debug_one_arg(c, asdl_seq_GET(args, i)));
    }
  }
  return SUCCESS;
}

static int compiler_check_debug_args(struct compiler *c, arguments_ty args) {
...
static int wrap_in_stopiteration_handler(struct compiler *c) {
...
static int compiler_type_param_bound_or_default(struct compiler *c, expr_ty e,
                                                identifier name, void *key,
                                                bool allow_starred) {
  if (compiler_enter_scope(c, name, COMPILER_SCOPE_TYPEPARAMS, key,
                           e->lineno) == -1) {
    return ERROR;
  }
...
static int compiler_type_params(struct compiler *c,
  ...
  for (Py_ssize_t i = 0; i < n; i++) {
    ...
    case TypeVar_kind:
      ...
      if (typeparam->v.TypeVar.bound) {
        ...
        if (compiler_type_param_bound_or_default(
      ...
      if (typeparam->v.TypeVar.default_value) {
        ...
        if (compiler_type_param_bound_or_default(
                c, default_, typeparam->v.TypeVar.name,
    ...
    case TypeVarTuple_kind:
      ...
      if (typeparam->v.TypeVarTuple.default_value) {
        ...
        if (compiler_type_param_bound_or_default(c, default_,
    ...
    case ParamSpec_kind:
      ...
      if (typeparam->v.ParamSpec.default_value) {
        ...
        if (compiler_type_param_bound_or_default(
                c, default_, typeparam->v.ParamSpec.name, (void *)typeparam,
                false) < 0) {
          return ERROR;
        }
        ADDOP_I(c, loc, CALL_INTRINSIC_2, INTRINSIC_SET_TYPEPARAM_DEFAULT);
...
static int compiler_function_body(struct compiler *c, stmt_ty s, int is_async,
                                  Py_ssize_t funcflags, int firstlineno) {
  arguments_ty args;
  identifier name;
  asdl_stmt_seq *body;
  int scope_type;
  ...
  if (compiler_add_const(c->c_const_cache, c->u,
                         docstring ? docstring : Py_None) < 0) {
  ...
  USE_LABEL(c, start);
  bool add_stopiteration_handler =
      c->u->u_ste->ste_coroutine || c->u->u_ste->ste_generator;
  if (add_stopiteration_handler) {
    /* wrap_in_stopiteration_handler will push a block, so we need to account
     * for that */
    RETURN_IF_ERROR(compiler_push_fblock(c, NO_LOCATION, STOP_ITERATION, start,
                                         NO_LABEL, NULL));
  }
...
static int compiler_function(struct compiler *c, stmt_ty s, int is_async) {
  arguments_ty args;
  expr_ty returns;
  identifier name;
  asdl_expr_seq *decos;
  ...
  if (is_generic) {
    ...
    for (int i = 0; i < num_typeparam_args; i++) {
      RETURN_IF_ERROR_IN_SCOPE(
          c, codegen_addop_i(INSTR_SEQUENCE(c), LOAD_FAST, i, loc));
    }
  }
  ...
  if (is_generic) {
    RETURN_IF_ERROR_IN_SCOPE(c,
                             codegen_addop_i(INSTR_SEQUENCE(c), SWAP, 2, loc));
    RETURN_IF_ERROR_IN_SCOPE(
        c, codegen_addop_i(INSTR_SEQUENCE(c), CALL_INTRINSIC_2,
                           INTRINSIC_SET_FUNCTION_TYPE_PARAMS, loc));
...
static int compiler_set_type_params_in_class(struct compiler *c, location loc) {
...
static int compiler_class_body(struct compiler *c, stmt_ty s, int firstlineno) {
  ...
  */
  ...
  RETURN_IF_ERROR(compiler_enter_scope(
      c, s->v.ClassDef.name, COMPILER_SCOPE_CLASS, (void *)s, firstlineno));
  ...
  if (compiler_nameop(c, NO_LOCATION, &_Py_ID(__static_attributes__), Store) <
      0) {
    compiler_exit_scope(c);
    return ERROR;
  }
  ...
  if (c->u->u_ste->ste_needs_classdict) {
    ...
    if (compiler_nameop(c, NO_LOCATION, &_Py_ID(__classdictcell__), Store) <
        0) {
      compiler_exit_scope(c);
      return ERROR;
    }
  ...
  if (c->u->u_ste->ste_needs_class_closure) {
    ...
    int i =
        compiler_lookup_arg(c->u->u_metadata.u_cellvars, &_Py_ID(__class__));
    if (i < 0) {
      compiler_exit_scope(c);
      return ERROR;
    }
...
static int compiler_class(struct compiler *c, stmt_ty s) {
  ...
  if (is_generic) {
    ...
    _Py_DECLARE_STR(type_params, ".type_params");
    RETURN_IF_ERROR_IN_SCOPE(
        c, compiler_nameop(c, loc, &_Py_STR(type_params), Store));
  }
  ...
  if (is_generic) {
    ...
    RETURN_IF_ERROR_IN_SCOPE(
        c, compiler_nameop(c, loc, &_Py_STR(type_params), Load));
    RETURN_IF_ERROR_IN_SCOPE(
        c, codegen_addop_i(INSTR_SEQUENCE(c), CALL_INTRINSIC_1,
                           INTRINSIC_SUBSCRIPT_GENERIC, loc))
    RETURN_IF_ERROR_IN_SCOPE(
        c, compiler_nameop(c, loc, &_Py_STR(generic_base), Store));

    Py_ssize_t original_len = asdl_seq_LEN(s->v.ClassDef.bases);
    asdl_expr_seq *bases = _Py_asdl_expr_seq_new(original_len + 1, c->c_arena);
    if (bases == NULL) {
      compiler_exit_scope(c);
      return ERROR;
    }
    ...
    expr_ty name_node =
        _PyAST_Name(&_Py_STR(generic_base), Load, loc.lineno, loc.col_offset,
                    loc.end_lineno, loc.end_col_offset, c->c_arena);
    if (name_node == NULL) {
      compiler_exit_scope(c);
      return ERROR;
    }
    ...
    RETURN_IF_ERROR_IN_SCOPE(
        c, compiler_call_helper(c, loc, 2, bases, s->v.ClassDef.keywords));
    ...
    RETURN_IF_ERROR(compiler_call_helper(c, loc, 2, s->v.ClassDef.bases,
...
static int compiler_typealias_body(struct compiler *c, stmt_ty s) {
  location loc = LOC(s);
...
static int compiler_typealias(struct compiler *c, stmt_ty s) {
  location loc = LOC(s);
...
static bool check_is_arg(expr_ty e) {
  if (e->kind != Constant_kind) {
    return true;
  }
  PyObject *value = e->v.Constant.value;
  return (value == Py_None || value == Py_False || value == Py_True ||
          value == Py_Ellipsis);
}
 ...
 */
static int check_compare(struct compiler *c, expr_ty e) {
  Py_ssize_t i, n;
...
static int compiler_addcompare(struct compiler *c, location loc, cmpop_ty op) {
  int cmp;
  switch (op) {
  case Eq:
    cmp = Py_EQ;
...
static int compiler_jump_if(struct compiler *c, location loc, expr_ty e,
                            jump_target_label next, int cond) {
  switch (e->kind) {
  case UnaryOp_kind:
  ...
  case IfExp_kind: {
    ...
    RETURN_IF_ERROR(compiler_jump_if(c, loc, e->v.IfExp.test, next2, 0));
    RETURN_IF_ERROR(compiler_jump_if(c, loc, e->v.IfExp.body, next, cond));
    ADDOP_JUMP(c, NO_LOCATION, JUMP_NO_INTERRUPT, end);

    USE_LABEL(c, next2);
    RETURN_IF_ERROR(compiler_jump_if(c, loc, e->v.IfExp.orelse, next, cond));
...
static int compiler_ifexp(struct compiler *c, expr_ty e) {
  assert(e->kind == IfExp_kind);
  NEW_JUMP_TARGET_LABEL(c, end);
  NEW_JUMP_TARGET_LABEL(c, next);

  RETURN_IF_ERROR(compiler_jump_if(c, LOC(e), e->v.IfExp.test, next, 0));
...
static int compiler_lambda(struct compiler *c, expr_ty e) {
  PyCodeObject *co;
  Py_ssize_t funcflags;
  ...
  RETURN_IF_ERROR(compiler_enter_scope(
      c, &_Py_STR(anon_lambda), COMPILER_SCOPE_LAMBDA, (void *)e, e->lineno));
...
static int compiler_if(struct compiler *c, stmt_ty s) {
  ...
  RETURN_IF_ERROR(compiler_jump_if(c, LOC(s), s->v.If.test, next, 0));
...
static int compiler_async_for(struct compiler *c, stmt_ty s) {
  location loc = LOC(s);
...
static int compiler_return(struct compiler *c, stmt_ty s) {
  location loc = LOC(s);
  ...
  if (s->v.Return.value != NULL && c->u->u_ste->ste_coroutine &&
...
static int compiler_nuke(struct compiler *c, location loc) { exit(45); }

static int compiler_break(struct compiler *c, location loc) {
...
static int compiler_continue(struct compiler *c, location loc) {
  struct fblockinfo *loop = NULL;
  location origin_loc = loc;
  /* Emit instruction with line number */
  ADDOP(c, loc, NOP);
...
static int compiler_try_finally(struct compiler *c, stmt_ty s) {
  location loc = LOC(s);
  ...
  USE_LABEL(c, body);
  RETURN_IF_ERROR(
      compiler_push_fblock(c, loc, FINALLY_TRY, body, end, s->v.Try.finalbody));
...
static int compiler_try_star_finally(struct compiler *c, stmt_ty s) {
  location loc = LOC(s);
  ...
  USE_LABEL(c, body);
  RETURN_IF_ERROR(compiler_push_fblock(c, loc, FINALLY_TRY, body, end,
...
*/
static int compiler_try_except(struct compiler *c, stmt_ty s) {
  location loc = LOC(s);
  Py_ssize_t i, n;
  ...
  RETURN_IF_ERROR(compiler_push_fblock(c, loc, EXCEPTION_HANDLER, NO_LABEL,
                                       NO_LABEL, NULL));

  for (i = 0; i < n; i++) {
    excepthandler_ty handler =
        (excepthandler_ty)asdl_seq_GET(s->v.Try.handlers, i);
    ...
    if (handler->v.ExceptHandler.name) {
      ...
      */
      ...
      RETURN_IF_ERROR(compiler_push_fblock(c, loc, HANDLER_CLEANUP,
                                           cleanup_body, NO_LABEL,
      ...
      ADDOP_LOAD_CONST(c, NO_LOCATION, Py_None);
      RETURN_IF_ERROR(compiler_nameop(c, NO_LOCATION,
                                      handler->v.ExceptHandler.name, Store));
      RETURN_IF_ERROR(
...
/*
   ...
   [exc]                            L1:       BUILD_LIST   )  list for
   raised/reraised excs ("result") [orig, res] COPY 2       )  make a copy of
   the original EG
   ...
   [orig, res, rest, i, v]          R1:       LIST_APPEND   3 ) exc raised in
   except* body - add to res [orig, res, rest, i]                       POP
   ...
   [orig, res, rest]                Ln+1:     LIST_APPEND 1  ) add unhandled exc
   to res (could be None)
   ...
   []                               L0:       <next statement>
*/
static int compiler_try_star_except(struct compiler *c, stmt_ty s) {
  location loc = LOC(s);
  ...
  /* Runtime will push a block here, so we need to account for that */
  RETURN_IF_ERROR(compiler_push_fblock(c, loc, EXCEPTION_GROUP_HANDLER,
  ...
  for (Py_ssize_t i = 0; i < n; i++) {
    ...
    */
    ...
    USE_LABEL(c, cleanup_body);
    RETURN_IF_ERROR(compiler_push_fblock(c, loc, HANDLER_CLEANUP, cleanup_body,
                                         NO_LABEL,
                                         handler->v.ExceptHandler.name));
...
static int compiler_try_star(struct compiler *c, stmt_ty s) {
...
static int compiler_import_as(struct compiler *c, location loc, identifier name,
                              identifier asname) {
...
static int compiler_import(struct compiler *c, stmt_ty s) {
  ...
  for (i = 0; i < n; i++) {
    ...
    if (alias->asname) {
      ...
      Py_ssize_t dot = PyUnicode_FindChar(alias->name, '.', 0,
                                          PyUnicode_GET_LENGTH(alias->name), 1);
      if (dot != -1) {
...
static int compiler_from_import(struct compiler *c, stmt_ty s) {
...
static int compiler_assert(struct compiler *c, stmt_ty s) {
  ...
  if ((s->v.Assert.test->kind == Tuple_kind &&
    ...
    RETURN_IF_ERROR(compiler_warn(c, LOC(s),
...
static int compiler_stmt_expr(struct compiler *c, location loc, expr_ty value) {
...
static int unaryop(unaryop_ty op) {
  switch (op) {
  case Invert:
    return UNARY_INVERT;
  case USub:
...
static int addop_binary(struct compiler *c, location loc, operator_ty binop,
                        bool inplace) {
  int oparg;
  switch (binop) {
  case Add:
...
static int addop_yield(struct compiler *c, location loc) {
...
static int compiler_nameop(struct compiler *c, location loc, identifier name,
                           expr_context_ty ctx) {
  int op, scope;
  Py_ssize_t arg;
  ...
  case LOCAL:
    if (_PyST_IsFunctionLike(c->u->u_ste)) {
      ...
      PyObject *item;
      if (PyDict_GetItemRef(c->u->u_metadata.u_fasthidden, mangled, &item) <
          0) {
        goto error;
      }
      if (item == Py_True) {
  ...
  case OP_DEREF:
    ...
    case Load:
      if (c->u->u_ste->ste_type == ClassBlock && !c->u->u_in_inlined_comp) {
        ...
        if (compiler_addop_o(c->u, loc, LOAD_DEREF, c->u->u_metadata.u_freevars,
                             &_Py_ID(__classdict__)) < 0) {
          goto error;
        }
      } else {
...
static int compiler_boolop(struct compiler *c, expr_ty e) {
  int jumpi;
  Py_ssize_t i, n;
  asdl_expr_seq *s;
...
static int starunpack_helper(struct compiler *c, location loc,
                             asdl_expr_seq *elts, int pushed, int build,
                             int add, int extend, int tuple) {
...
static int unpack_helper(struct compiler *c, location loc,
                         asdl_expr_seq *elts) {
  Py_ssize_t n = asdl_seq_LEN(elts);
  int seen_star = 0;
  for (Py_ssize_t i = 0; i < n; i++) {
...
static int assignment_helper(struct compiler *c, location loc,
                             asdl_expr_seq *elts) {
  Py_ssize_t n = asdl_seq_LEN(elts);
...
static int compiler_list(struct compiler *c, expr_ty e) {
  location loc = LOC(e);
  asdl_expr_seq *elts = e->v.List.elts;
  if (e->v.List.ctx == Store) {
    return assignment_helper(c, loc, elts);
  } else if (e->v.List.ctx == Load) {
    return starunpack_helper(c, loc, elts, 0, BUILD_LIST, LIST_APPEND,
                             LIST_EXTEND, 0);
  } else {
...
static int compiler_tuple(struct compiler *c, expr_ty e) {
  location loc = LOC(e);
...
static int compiler_set(struct compiler *c, expr_ty e) {
  location loc = LOC(e);
  return starunpack_helper(c, loc, e->v.Set.elts, 0, BUILD_SET, SET_ADD,
                           SET_UPDATE, 0);
}

static bool are_all_items_const(asdl_expr_seq *seq, Py_ssize_t begin,
                                Py_ssize_t end) {
...
static int compiler_subdict(struct compiler *c, expr_ty e, Py_ssize_t begin,
                            Py_ssize_t end) {
...
static int compiler_dict(struct compiler *c, expr_ty e) {
  location loc = LOC(e);
...
static int compiler_compare(struct compiler *c, expr_ty e) {
  location loc = LOC(e);
  Py_ssize_t i, n;
...
static int check_caller(struct compiler *c, expr_ty e) {
  switch (e->kind) {
  case Constant_kind:
  case Tuple_kind:
  case List_kind:
...
static int check_subscripter(struct compiler *c, expr_ty e) {
  PyObject *v;
...
static int check_index(struct compiler *c, expr_ty e, expr_ty s) {
  PyObject *v;
  ...
  case FormattedValue_kind: {
    ...
    return compiler_warn(c, loc,
                         ...
                         "or slices, not %.200s; "
                         "perhaps you missed a comma?",
                         infer_type(e)->tp_name, index_type->tp_name);
  }
  default:
    return SUCCESS;
  }
...
static int is_import_originated(struct compiler *c, expr_ty e) {
...
static int can_optimize_super_call(struct compiler *c, expr_ty attr) {
...
static int load_args_for_super(struct compiler *c, expr_ty e) {
  location loc = LOC(e);
...
static location update_start_location_to_match_attr(struct compiler *c,
                                                    location loc,
                                                    expr_ty attr) {
...
static int maybe_optimize_method_call(struct compiler *c, expr_ty e) {
  ...
  if (kwdsl) {
    ...
    RETURN_IF_ERROR(compiler_call_simple_kw_helper(c, loc, kwds, kwdsl));
  ...
  for (Py_ssize_t i = 0; i < nkeywords; i++) {
    ...
    for (Py_ssize_t j = i + 1; j < nkeywords; j++) {
      ...
      if (other->arg && !PyUnicode_Compare(key->arg, other->arg)) {
        compiler_error(c, LOC(other), "keyword argument repeated: %U",
                       key->arg);
        return ERROR;
      }
    }
...
static int compiler_call(struct compiler *c, expr_ty e) {
  ...
  ADDOP(c, loc, PUSH_NULL);
  loc = LOC(e);
  return compiler_call_helper(c, loc, 0, e->v.Call.args, e->v.Call.keywords);
}

static int compiler_joined_str(struct compiler *c, expr_ty e) {
  location loc = LOC(e);
...
static int compiler_formatted_value(struct compiler *c, expr_ty e) {
  ...
  */
  ...
  if (conversion != -1) {
    ...
    case 'a':
      ...
      break;
    default:
      PyErr_Format(PyExc_SystemError, "Unrecognized conversion character %d",
                   conversion);
      return ERROR;
    }
...
static int compiler_subkwargs(struct compiler *c, location loc,
                              asdl_keyword_seq *keywords, Py_ssize_t begin,
                              Py_ssize_t end) {
  Py_ssize_t i, n = end - begin;
  keyword_ty kw;
  PyObject *keys, *key;
...
static int compiler_call_helper(struct compiler *c, location loc,
  ...
  if (nkwelts) {
    ...
    RETURN_IF_ERROR(compiler_call_simple_kw_helper(c, loc, keywords, nkwelts));
...
ex_call:
  ...
  if (n == 0 && nelts == 1 &&
    ...
    RETURN_IF_ERROR(starunpack_helper(c, loc, args, n, BUILD_LIST, LIST_APPEND,
                                      LIST_EXTEND, 1));
  }
...
static int compiler_comprehension_generator(struct compiler *c, location loc,
                                            asdl_comprehension_seq *generators,
...
static int compiler_sync_comprehension_generator(
  ...
  if (++gen_index < asdl_seq_LEN(generators)) {
    RETURN_IF_ERROR(compiler_comprehension_generator(
  ...
  if (gen_index >= asdl_seq_LEN(generators)) {
    ...
    case COMP_DICTCOMP:
      ...
      VISIT(c, expr, elt);
      VISIT(c, expr, val);
      elt_loc = LOCATION(elt->lineno, val->end_lineno, elt->col_offset,
                         val->end_col_offset);
...
static int compiler_async_comprehension_generator(
  ...
  USE_LABEL(c, start);
  /* Runtime will push a block here, so we need to account for that */
  RETURN_IF_ERROR(compiler_push_fblock(c, loc, ASYNC_COMPREHENSION_GENERATOR,
                                       start, NO_LABEL, NULL));
...
push_inlined_comprehension_state(struct compiler *c, location loc,
  ...
  while (PyDict_Next(entry->ste_symbols, &pos, &k, &v)) {
    ...
    // either way.)
    if ((scope != outsc && scope != FREE &&
         !(scope == CELL && outsc == FREE)) ||
        in_class_block) {
...
static int pop_inlined_comprehension_state(struct compiler *c, location loc,
                                           inlined_comprehension_state state) {
  c->u->u_in_inlined_comp--;
  PyObject *k, *v;
  Py_ssize_t pos = 0;
...
static inline int compiler_comprehension_iter(struct compiler *c, location loc,
...
static int compiler_comprehension(struct compiler *c, expr_ty e, int type,
                                  identifier name,
                                  asdl_comprehension_seq *generators,
  ...
  if (is_inlined) {
    ...
    if (compiler_enter_scope(c, name, COMPILER_SCOPE_COMPREHENSION, (void *)e,
                             e->lineno) < 0) {
      goto error;
    }
  }
  ...
  if (is_async_generator && type != COMP_GENEXP &&
      scope_type != COMPILER_SCOPE_ASYNC_FUNCTION &&
      scope_type != COMPILER_SCOPE_COMPREHENSION && !is_top_level_await) {
  ...
  if (compiler_comprehension_generator(c, loc, generators, 0, 0, elt, val, type,
                                       is_inlined) < 0) {
    goto error_in_scope;
  }
...
static int compiler_genexp(struct compiler *c, expr_ty e) {
...
static int compiler_listcomp(struct compiler *c, expr_ty e) {
  assert(e->kind == ListComp_kind);
  _Py_DECLARE_STR(anon_listcomp, "<listcomp>");
  return compiler_comprehension(c, e, COMP_LISTCOMP, &_Py_STR(anon_listcomp),
                                e->v.ListComp.generators, e->v.ListComp.elt,
                                NULL);
}

static int compiler_setcomp(struct compiler *c, expr_ty e) {
  assert(e->kind == SetComp_kind);
  _Py_DECLARE_STR(anon_setcomp, "<setcomp>");
  return compiler_comprehension(c, e, COMP_SETCOMP, &_Py_STR(anon_setcomp),
                                e->v.SetComp.generators, e->v.SetComp.elt,
                                NULL);
}

static int compiler_dictcomp(struct compiler *c, expr_ty e) {
  assert(e->kind == DictComp_kind);
  _Py_DECLARE_STR(anon_dictcomp, "<dictcomp>");
  return compiler_comprehension(c, e, COMP_DICTCOMP, &_Py_STR(anon_dictcomp),
                                e->v.DictComp.generators, e->v.DictComp.key,
                                e->v.DictComp.value);
}

static int compiler_visit_keyword(struct compiler *c, keyword_ty k) {
  VISIT(c, expr, k->value);
  return SUCCESS;
}

static int compiler_with_except_finish(struct compiler *c,
                                       jump_target_label cleanup) {
...
/*
   ...
   try:
       ...
       if not (await exit(*exc)):
           raise
 */
static int compiler_async_with(struct compiler *c, stmt_ty s, int pos) {
  location loc = LOC(s);
...
static int compiler_with(struct compiler *c, stmt_ty s, int pos) {
...
static int compiler_visit_expr1(struct compiler *c, expr_ty e) {
  ...
  case Starred_kind:
    ...
    default:
      return compiler_error(c, loc, "can't use starred expression here");
    }
    break;
  case Slice_kind: {
...
static bool is_two_element_slice(expr_ty s) {
  return s->kind == Slice_kind && s->v.Slice.step == NULL;
}

static int compiler_augassign(struct compiler *c, stmt_ty s) {
  ...
  default:
    PyErr_Format(PyExc_SystemError,
                 "invalid node type (%d) for augmented assignment", e->kind);
    return ERROR;
  }
...
static int check_ann_expr(struct compiler *c, expr_ty e) {
  VISIT(c, expr, e);
  ADDOP(c, LOC(e), POP_TOP);
  return SUCCESS;
}

static int check_annotation(struct compiler *c, stmt_ty s) {
...
static int check_ann_subscr(struct compiler *c, expr_ty e) {
...
static int compiler_annassign(struct compiler *c, stmt_ty s) {
  location loc = LOC(s);
  ...
  case Name_kind:
    ...
    if (s->v.AnnAssign.simple && (c->u->u_scope_type == COMPILER_SCOPE_MODULE ||
                                  c->u->u_scope_type == COMPILER_SCOPE_CLASS)) {
      if (c->c_future.ff_features & CO_FUTURE_ANNOTATIONS) {
  ...
  default:
    PyErr_Format(PyExc_SystemError,
                 "invalid node type (%d) for annotated assignment", targ->kind);
    return ERROR;
  }
...
static int compiler_error(struct compiler *c, location loc, const char *format,
                          ...) {
  va_list vargs;
  ...
  PyObject *args = Py_BuildValue("O(OiiOii)", msg, c->c_filename, loc.lineno,
                                 loc.col_offset + 1, loc_obj, loc.end_lineno,
                                 loc.end_col_offset + 1);
  Py_DECREF(msg);
  if (args == NULL) {
    goto exit;
...
/* Emits a SyntaxWarning and returns 1 on success.
   ...
   and returns 0.
*/
static int compiler_warn(struct compiler *c, location loc, const char *format,
                         ...) {
  va_list vargs;
  va_start(vargs, format);
  PyObject *msg = PyUnicode_FromFormatV(format, vargs);
...
static int compiler_subscript(struct compiler *c, expr_ty e) {
  location loc = LOC(e);
...
static int compiler_slice(struct compiler *c, expr_ty s) {
  int n = 2;
...
// Limit permitted subexpressions, even if the parser & AST validator let them
// through
#define MATCH_VALUE_EXPR(N)                                                    \
  ((N)->kind == Constant_kind || (N)->kind == Attribute_kind)

// Allocate or resize pc->fail_pop to allow for n items to be popped on failure.
static int ensure_fail_pop(struct compiler *c, pattern_context *pc,
                           Py_ssize_t n) {
  Py_ssize_t size = n + 1;
...
static int jump_to_fail_pop(struct compiler *c, location loc,
...
static int emit_and_reset_fail_pop(struct compiler *c, location loc,
                                   pattern_context *pc) {
  if (!pc->fail_pop_size) {
...
static int compiler_error_duplicate_store(struct compiler *c, location loc,
                                          identifier n) {
  return compiler_error(c, loc, "multiple assignments to name %R in pattern",
                        n);
}

// Duplicate the effect of 3.10's ROT_* instructions using SWAPs.
static int pattern_helper_rotate(struct compiler *c, location loc,
                                 Py_ssize_t count) {
  while (1 < count) {
...
static int pattern_helper_store_name(struct compiler *c, location loc,
...
static int pattern_unpack_helper(struct compiler *c, location loc,
...
static int pattern_helper_sequence_unpack(struct compiler *c, location loc,
...
static int pattern_helper_sequence_subscr(struct compiler *c, location loc,
                                          asdl_pattern_seq *patterns,
                                          Py_ssize_t star,
                                          pattern_context *pc) {
...
static int compiler_pattern_subpattern(struct compiler *c, pattern_ty p,
                                       pattern_context *pc) {
...
static int compiler_pattern_as(struct compiler *c, pattern_ty p,
                               pattern_context *pc) {
...
static int compiler_pattern_star(struct compiler *c, pattern_ty p,
                                 pattern_context *pc) {
...
static int validate_kwd_attrs(struct compiler *c, asdl_identifier_seq *attrs,
                              asdl_pattern_seq *patterns) {
  ...
  for (Py_ssize_t i = 0; i < nattrs; i++) {
    ...
    for (Py_ssize_t j = i + 1; j < nattrs; j++) {
      ...
      if (!PyUnicode_Compare(attr, other)) {
        ...
        compiler_error(c, loc, "attribute name repeated in class pattern: %U",
                       attr);
        return ERROR;
      }
    }
...
static int compiler_pattern_class(struct compiler *c, pattern_ty p,
                                  pattern_context *pc) {
...
static int compiler_pattern_mapping(struct compiler *c, pattern_ty p,
                                    pattern_context *pc) {
...
static int compiler_pattern_or(struct compiler *c, pattern_ty p,
                               pattern_context *pc) {
...
static int compiler_pattern_sequence(struct compiler *c, pattern_ty p,
                                     pattern_context *pc) {
...
static int compiler_pattern_value(struct compiler *c, pattern_ty p,
                                  pattern_context *pc) {
...
static int compiler_pattern_singleton(struct compiler *c, pattern_ty p,
                                      pattern_context *pc) {
...
static int compiler_match_inner(struct compiler *c, stmt_ty s,
                                pattern_context *pc) {
  ...
  for (Py_ssize_t i = 0; i < cases - has_default; i++) {
    ...
    if (m->guard) {
      ...
      RETURN_IF_ERROR(
          compiler_jump_if(c, LOC(m->pattern), m->guard, pc->fail_pop[0], 0));
    }
...
static int add_return_at_end(struct compiler *c, int addNone) {
...
static PyCodeObject *optimize_and_assemble_code_unit(struct compiler_unit *u,
  ...
  if (_PyCfg_OptimizeCodeUnit(g, consts, const_cache, nlocals, nparams,
...
static PyCodeObject *optimize_and_assemble(struct compiler *c, int addNone) {
  struct compiler_unit *u = c->u;
...
static PyObject *cfg_to_instruction_sequence(cfg_builder *g) {
...
PyObject *_PyCompile_CleanDoc(PyObject *doc) {
...
PyObject *_PyCompile_OptimizeCfg(PyObject *seq, PyObject *consts, int nlocals) {
...
PyCodeObject *_PyCompile_Assemble(_PyCompile_CodeUnitMetadata *umd,
                                  PyObject *filename, PyObject *seq) {
  if (!_PyInstructionSequence_Check(seq)) {
  ...
  if (_PyCfg_OptimizedCfgToInstructionSequence(g, umd, code_flags, &stackdepth,
                                               &nlocalsplus,
                                               &optimized_instrs) < 0) {
    goto error;
  }
  ...
  co = _PyAssemble_MakeCodeObject(umd, const_cache, consts, stackdepth,
                                  &optimized_instrs, nlocalsplus, code_flags,
                                  filename);
  Py_DECREF(consts);
...
PyObject *PyCode_Optimize(PyObject *code, PyObject *Py_UNUSED(consts),
