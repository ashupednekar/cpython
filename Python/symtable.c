...
#define NAMED_EXPR_COMP_IN_CLASS                                               \
  "assignment expression within a comprehension cannot be used in a class "    \
  "body"

#define NAMED_EXPR_COMP_IN_TYPEVAR_BOUND                                       \
  "assignment expression within a comprehension cannot be used in a TypeVar "  \
  "bound"

#define NAMED_EXPR_COMP_IN_TYPEALIAS                                           \
  "assignment expression within a comprehension cannot be used in a type "     \
  "alias"
...
#define TYPEVAR_BOUND_NOT_ALLOWED "%s cannot be used within a TypeVar bound"

#define TYPEALIAS_NOT_ALLOWED "%s cannot be used within a type alias"
...
#define ST_LOCATION(x)                                                         \
  (x)->ste_lineno, (x)->ste_col_offset, (x)->ste_end_lineno,                   \
      (x)->ste_end_col_offset

static PySTEntryObject *ste_new(struct symtable *st, identifier name,
                                _Py_block_ty block, void *key, int lineno,
...
static PyObject *ste_repr(PySTEntryObject *ste) {
  return PyUnicode_FromFormat("<symtable entry %U(%R), line %d>", ste->ste_name,
                              ste->ste_id, ste->ste_lineno);
}
...
static int symtable_visit_annotations(struct symtable *st, stmt_ty,
                                      arguments_ty, expr_ty);
static int symtable_visit_withitem(struct symtable *st, withitem_ty item);
static int symtable_visit_match_case(struct symtable *st, match_case_ty m);
static int symtable_visit_pattern(struct symtable *st, pattern_ty s);
static int symtable_raise_if_annotation_block(struct symtable *st, const char *,
                                              expr_ty);
...
static void _dump_symtable(PySTEntryObject *ste, PyObject *prefix) {
  ...
  case TypeVarBoundBlock:
    blocktype = "TypeVarBoundBlock";
    break;
  case TypeAliasBlock:
    blocktype = "TypeAliasBlock";
    break;
  case TypeParamBlock:
    blocktype = "TypeParamBlock";
    break;
  }
  const char *comptype = "";
  ...
  PyObject *msg = PyUnicode_FromFormat(
      ("%U=== Symtable for %U ===\n"
       ...
       "%U--- Symbols ---\n"),
      prefix, ste->ste_name, prefix, blocktype, comptype, prefix,
      ste->ste_nested ? " nested" : "", ste->ste_free ? " free" : "",
      ...
      ste->ste_can_see_class_scope ? " can_see_class_scope" : "", prefix,
      ste->ste_lineno, ste->ste_col_offset, prefix);
  assert(msg != NULL);
...
static struct symtable *symtable_new(void) {
  struct symtable *st;
...
struct symtable *_PySymtable_Build(mod_ty mod, PyObject *filename,
  ...
  if (!symtable_enter_block(st, &_Py_ID(top), ModuleBlock, (void *)mod, 0, 0, 0,
                            0)) {
    _PySymtable_Free(st);
    return NULL;
  }
...
void _PySymtable_Free(struct symtable *st) {
...
PySTEntryObject *_PySymtable_Lookup(struct symtable *st, void *key) {
  PyObject *k, *v;
...
long _PyST_GetSymbol(PySTEntryObject *ste, PyObject *name) {
...
int _PyST_GetScope(PySTEntryObject *ste, PyObject *name) {
  long symbol = _PyST_GetSymbol(ste, name);
  return (symbol >> SCOPE_OFFSET) & SCOPE_MASK;
}

int _PyST_IsFunctionLike(PySTEntryObject *ste) {
  return ste->ste_type == FunctionBlock || ste->ste_type == TypeVarBoundBlock ||
         ste->ste_type == TypeAliasBlock || ste->ste_type == TypeParamBlock;
}

static int error_at_directive(PySTEntryObject *ste, PyObject *name) {
  Py_ssize_t i;
  PyObject *data;
  ...
  for (i = 0; i < PyList_GET_SIZE(ste->ste_directives); i++) {
    ...
    if (PyUnicode_Compare(PyTuple_GET_ITEM(data, 0), name) == 0) {
      PyErr_RangedSyntaxLocationObject(
          ste->ste_table->st_filename, PyLong_AsLong(PyTuple_GET_ITEM(data, 1)),
...
/* Analyze raw symbol information to determine scope of each name.
   ...
   During analysis, the names are:
      symbols: dict mapping from symbol names to flag values (including offset
   scope values) scopes: dict mapping from symbol names to scope values (no
   offset) local: set of all symbol names local to the current scope bound: set
   of all symbol names local to a containing function scope free: set of all
   symbol names referenced but not bound in child scopes global: set of all
   symbol names explicitly declared as global
*/
...
static int analyze_name(PySTEntryObject *ste, PyObject *scopes, PyObject *name,
  ...
  if (flags & DEF_NONLOCAL) {
    ...
    if (!contains) {
      PyErr_Format(PyExc_SyntaxError, "no binding for nonlocal '%U' found",
                   name);
  ...
  // If we were passed class_entry (i.e., we're in an ste_can_see_class_scope
  // scope) and the bound name is in that set, then the name is potentially
  // bound both by the immediately enclosing class namespace, and also by an
  // outer function namespace. In that case, we want the runtime name resolution
  // to look at only the class namespace and the globals (not the namespace
  // providing the bound). Similarly, if the name is explicitly global in the
  // class namespace (through the global statement), we want to also treat it as
  // a global in this scope.
  if (class_entry != NULL) {
...
static int is_free_in_any_child(PySTEntryObject *entry, PyObject *key) {
...
static int inline_comprehension(PySTEntryObject *ste, PySTEntryObject *comp,
...
static int analyze_cells(PyObject *scopes, PyObject *free,
                         PyObject *inlined_cells) {
...
static int drop_class_free(PySTEntryObject *ste, PyObject *free) {
  int res;
 ...
 */
static int update_symbols(PyObject *symbols, PyObject *scopes, PyObject *bound,
                          PyObject *free, PyObject *inlined_cells,
                          int classflag) {
...
static int analyze_child_block(PySTEntryObject *entry, PyObject *bound,
...
static int analyze_block(PySTEntryObject *ste, PyObject *bound, PyObject *free,
  ...
  */
  for (i = 0; i < PyList_GET_SIZE(ste->ste_children); ++i) {
    ...
    if (!analyze_child_block(entry, newbound, newfree, newglobal, type_params,
                             new_class_entry, &child_free)) {
      goto error;
    }
    if (inline_comp) {
...
static int symtable_enter_block(struct symtable *st, identifier name,
  ...
  ste = ste_new(st, name, block, ast, lineno, col_offset, end_lineno,
                end_col_offset);
  if (ste == NULL)
    return 0;
...
static long symtable_lookup_entry(struct symtable *st, PySTEntryObject *ste,
                                  PyObject *name) {
...
static int symtable_add_def_helper(struct symtable *st, PyObject *name,
                                   int flag, struct _symtable_entry *ste,
  ...
  if ((o = PyDict_GetItemWithError(dict, mangled))) {
    ...
    if ((flag & DEF_PARAM) && (val & DEF_PARAM)) {
      ...
      PyErr_RangedSyntaxLocationObject(st->st_filename, lineno, col_offset + 1,
  ...
  if (ste->ste_comp_iter_target) {
    ...
    if (val & (DEF_GLOBAL | DEF_NONLOCAL)) {
      PyErr_Format(PyExc_SyntaxError, NAMED_EXPR_COMP_INNER_LOOP_CONFLICT,
                   name);
      PyErr_RangedSyntaxLocationObject(st->st_filename, lineno, col_offset + 1,
                                       end_lineno, end_col_offset + 1);
      goto error;
...
static int symtable_add_def(struct symtable *st, PyObject *name, int flag,
                            int lineno, int col_offset, int end_lineno,
                            int end_col_offset) {
  return symtable_add_def_helper(st, name, flag, st->st_cur, lineno, col_offset,
                                 end_lineno, end_col_offset);
}

static int symtable_enter_type_param_block(struct symtable *st, identifier name,
                                           void *ast, int has_defaults,
                                           int has_kwdefaults,
                                           enum _stmt_kind kind, int lineno,
                                           int col_offset, int end_lineno,
                                           int end_col_offset) {
  _Py_block_ty current_type = st->st_cur->ste_type;
  if (!symtable_enter_block(st, name, TypeParamBlock, ast, lineno, col_offset,
                            end_lineno, end_col_offset)) {
    return 0;
  }
  if (current_type == ClassBlock) {
    st->st_cur->ste_can_see_class_scope = 1;
    if (!symtable_add_def(st, &_Py_ID(__classdict__), USE, lineno, col_offset,
                          end_lineno, end_col_offset)) {
      return 0;
    }
  }
  if (kind == ClassDef_kind) {
    ...
    if (!symtable_add_def(st, &_Py_STR(type_params), DEF_LOCAL, lineno,
                          col_offset, end_lineno, end_col_offset)) {
      return 0;
    }
    if (!symtable_add_def(st, &_Py_STR(type_params), USE, lineno, col_offset,
                          end_lineno, end_col_offset)) {
      return 0;
    }
    st->st_private = name;
    ...
    if (!symtable_add_def(st, &_Py_STR(generic_base), DEF_LOCAL, lineno,
                          col_offset, end_lineno, end_col_offset)) {
      return 0;
    }
    if (!symtable_add_def(st, &_Py_STR(generic_base), USE, lineno, col_offset,
                          end_lineno, end_col_offset)) {
      return 0;
    }
  }
  if (has_defaults) {
    ...
    if (!symtable_add_def(st, &_Py_STR(defaults), DEF_PARAM, lineno, col_offset,
                          end_lineno, end_col_offset)) {
      return 0;
    }
  }
  if (has_kwdefaults) {
    ...
    if (!symtable_add_def(st, &_Py_STR(kwdefaults), DEF_PARAM, lineno,
                          col_offset, end_lineno, end_col_offset)) {
      return 0;
    }
  }
...
/* VISIT, VISIT_SEQ and VIST_SEQ_TAIL take an ASDL type as their second
   argument. They use the ASDL name to synthesize the name of the C type and the
   visit function.
...
#define VISIT_QUIT(ST, X) return --(ST)->recursion_depth, (X)
...
static int symtable_record_directive(struct symtable *st, identifier name,
                                     int lineno, int col_offset, int end_lineno,
                                     int end_col_offset) {
...
static int has_kwonlydefaults(asdl_arg_seq *kwonlyargs,
...
static int symtable_visit_stmt(struct symtable *st, stmt_ty s) {
  ...
  case FunctionDef_kind:
    ...
    if (asdl_seq_LEN(s->v.FunctionDef.type_params) > 0) {
      if (!symtable_enter_type_param_block(
              st, s->v.FunctionDef.name, (void *)s->v.FunctionDef.type_params,
    ...
    if (!symtable_visit_annotations(st, s, s->v.FunctionDef.args,
      ...
      VISIT_QUIT(st, 0);
    if (!symtable_enter_block(st, s->v.FunctionDef.name, FunctionBlock,
                              (void *)s, LOCATION(s)))
      VISIT_QUIT(st, 0);
  ...
  case ClassDef_kind: {
    ...
    if (asdl_seq_LEN(s->v.ClassDef.type_params) > 0) {
      if (!symtable_enter_type_param_block(
              st, s->v.ClassDef.name, (void *)s->v.ClassDef.type_params, false,
              false, s->kind, LOCATION(s))) {
        VISIT_QUIT(st, 0);
      }
    ...
    if (!symtable_enter_block(st, s->v.ClassDef.name, ClassBlock, (void *)s,
    ...
    if (asdl_seq_LEN(s->v.ClassDef.type_params) > 0) {
      if (!symtable_add_def(st, &_Py_ID(__type_params__), DEF_LOCAL,
                            LOCATION(s))) {
        VISIT_QUIT(st, 0);
      }
      _Py_DECLARE_STR(type_params, ".type_params");
  ...
  case TypeAlias_kind: {
    ...
    if (is_generic) {
      if (!symtable_enter_type_param_block(
              st, name, (void *)s->v.TypeAlias.type_params, false, false,
              s->kind, LOCATION(s))) {
        VISIT_QUIT(st, 0);
      }
      VISIT_SEQ(st, type_param, s->v.TypeAlias.type_params);
    }
    if (!symtable_enter_block(st, name, TypeAliasBlock, (void *)s, LOCATION(s)))
      VISIT_QUIT(st, 0);
    st->st_cur->ste_can_see_class_scope = is_in_class;
    if (is_in_class && !symtable_add_def(st, &_Py_ID(__classdict__), USE,
                                         LOCATION(s->v.TypeAlias.value))) {
      VISIT_QUIT(st, 0);
    }
  ...
  case AnnAssign_kind:
    if (s->v.AnnAssign.target->kind == Name_kind) {
      ...
      if ((cur & (DEF_GLOBAL | DEF_NONLOCAL)) &&
          (st->st_cur->ste_symbols != st->st_global) && s->v.AnnAssign.simple) {
        PyErr_Format(PyExc_SyntaxError,
                     cur & DEF_GLOBAL ? GLOBAL_ANNOT : NONLOCAL_ANNOT,
                     e_name->v.Name.id);
        PyErr_RangedSyntaxLocationObject(st->st_filename, s->lineno,
                                         s->col_offset + 1, s->end_lineno,
                                         s->end_col_offset + 1);
        VISIT_QUIT(st, 0);
      }
      if (s->v.AnnAssign.simple &&
          !symtable_add_def(st, e_name->v.Name.id, DEF_ANNOT | DEF_LOCAL,
                            LOCATION(e_name))) {
        VISIT_QUIT(st, 0);
      } else {
  ...
  case AsyncFunctionDef_kind:
    ...
    if (!symtable_visit_annotations(st, s, s->v.AsyncFunctionDef.args,
      ...
      VISIT_QUIT(st, 0);
    if (!symtable_enter_block(st, s->v.AsyncFunctionDef.name, FunctionBlock,
...
static int symtable_extend_namedexpr_scope(struct symtable *st, expr_ty e) {
  assert(st->st_stack);
  ...
  for (i = size - 1; i >= 0; i--) {
    ...
    if (ste->ste_type == ClassBlock || ste->ste_type == TypeParamBlock ||
        ste->ste_type == TypeAliasBlock || ste->ste_type == TypeVarBoundBlock) {
      switch (ste->ste_type) {
      case ClassBlock:
  ...
  /* We should always find either a function-like block, ModuleBlock or
     ClassBlock and should never fall to this case
  */
  Py_UNREACHABLE();
  return 0;
}
...
static int symtable_visit_expr(struct symtable *st, expr_ty e) {
  ...
  case Lambda_kind: {
    ...
    if (!symtable_enter_block(st, &_Py_ID(lambda), FunctionBlock, (void *)e,
                              e->lineno, e->col_offset, e->end_lineno,
                              e->end_col_offset))
      VISIT_QUIT(st, 0);
...
static int symtable_visit_type_param_bound_or_default(struct symtable *st,
                                                      expr_ty e,
                                                      identifier name,
                                                      void *key) {
  if (e) {
...
static int symtable_visit_type_param(struct symtable *st, type_param_ty tp) {
  if (++st->recursion_depth > st->recursion_limit) {
    PyErr_SetString(PyExc_RecursionError,
                    "maximum recursion depth exceeded during compilation");
    VISIT_QUIT(st, 0);
  ...
  switch (tp->kind) {
  case TypeVar_kind:
    if (!symtable_add_def(st, tp->v.TypeVar.name, DEF_TYPE_PARAM | DEF_LOCAL,
                          LOCATION(tp)))
      VISIT_QUIT(st, 0);
    ...
    if (!symtable_visit_type_param_bound_or_default(
            st, tp->v.TypeVar.bound, tp->v.TypeVar.name, (void *)tp)) {
      VISIT_QUIT(st, 0);
    }
    if (!symtable_visit_type_param_bound_or_default(
            st, tp->v.TypeVar.default_value, tp->v.TypeVar.name,
            (void *)((uintptr_t)tp + 1))) {
      VISIT_QUIT(st, 0);
    }
    break;
  case TypeVarTuple_kind:
    if (!symtable_add_def(st, tp->v.TypeVarTuple.name,
                          DEF_TYPE_PARAM | DEF_LOCAL, LOCATION(tp))) {
      VISIT_QUIT(st, 0);
    }
    if (!symtable_visit_type_param_bound_or_default(
            st, tp->v.TypeVarTuple.default_value, tp->v.TypeVarTuple.name,
            (void *)tp)) {
      VISIT_QUIT(st, 0);
    }
    break;
  case ParamSpec_kind:
    if (!symtable_add_def(st, tp->v.ParamSpec.name, DEF_TYPE_PARAM | DEF_LOCAL,
                          LOCATION(tp))) {
      VISIT_QUIT(st, 0);
    }
    if (!symtable_visit_type_param_bound_or_default(
            st, tp->v.ParamSpec.default_value, tp->v.ParamSpec.name,
            (void *)tp)) {
      VISIT_QUIT(st, 0);
    }
    break;
...
static int symtable_visit_pattern(struct symtable *st, pattern_ty p) {
  if (++st->recursion_depth > st->recursion_limit) {
    PyErr_SetString(PyExc_RecursionError,
                    "maximum recursion depth exceeded during compilation");
    VISIT_QUIT(st, 0);
...
static int symtable_visit_annotations(struct symtable *st, stmt_ty o,
...
static int symtable_visit_arguments(struct symtable *st, arguments_ty a) {
...
static int symtable_visit_alias(struct symtable *st, alias_ty a) {
  ...
  */
  ...
  Py_ssize_t dot =
      PyUnicode_FindChar(name, '.', 0, PyUnicode_GET_LENGTH(name), 1);
  if (dot != -1) {
...
static int symtable_handle_comprehension(struct symtable *st, expr_ty e,
  ...
  if (!scope_name ||
      !symtable_enter_block(st, scope_name, FunctionBlock, (void *)e, e->lineno,
                            e->col_offset, e->end_lineno, e->end_col_offset)) {
    return 0;
  }
  switch (e->kind) {
  case ListComp_kind:
...
static int symtable_visit_genexp(struct symtable *st, expr_ty e) {
...
static int symtable_raise_if_annotation_block(struct symtable *st,
                                              const char *name, expr_ty e) {
...
static int symtable_raise_if_comprehension_block(struct symtable *st,
                                                 expr_ty e) {
...
struct symtable *_Py_SymtableStringObjectFlags(const char *str,
