...
static int has_starred(asdl_expr_seq *elts) {
...
static Py_ssize_t check_complexity(PyObject *obj, Py_ssize_t limit) {
...
static PyObject *safe_multiply(PyObject *v, PyObject *w) {
  if (PyLong_Check(v) && PyLong_Check(w) &&
      !_PyLong_IsZero((PyLongObject *)v) &&
...
static PyObject *safe_power(PyObject *v, PyObject *w) {
  if (PyLong_Check(v) && PyLong_Check(w) &&
      !_PyLong_IsZero((PyLongObject *)v) &&
...
static PyObject *safe_lshift(PyObject *v, PyObject *w) {
  if (PyLong_Check(v) && PyLong_Check(w) &&
      !_PyLong_IsZero((PyLongObject *)v) &&
      !_PyLong_IsZero((PyLongObject *)w)) {
    size_t vbits = _PyLong_NumBits(v);
...
static PyObject *safe_mod(PyObject *v, PyObject *w) {
...
static expr_ty parse_literal(PyObject *fmt, Py_ssize_t *ppos, PyArena *arena) {
...
static int simple_format_arg_parse(PyObject *fmt, Py_ssize_t *ppos, int *spec,
...
static expr_ty parse_format(PyObject *fmt, Py_ssize_t *ppos, expr_ty arg,
                            PyArena *arena) {
  ...
  if (spec == 's' || spec == 'r' || spec == 'a') {
    ...
    return _PyAST_FormattedValue(arg, spec, format_spec, arg->lineno,
                                 arg->col_offset, arg->end_lineno,
                                 arg->end_col_offset, arena);
  }
  // Unsupported format.
  return NULL;
}

static int optimize_format(expr_ty node, PyObject *fmt, asdl_expr_seq *elts,
                           PyArena *arena) {
  Py_ssize_t pos = 0;
  Py_ssize_t cnt = 0;
  ...
  expr_ty res = _PyAST_JoinedStr(seq, node->lineno, node->col_offset,
...
static int fold_binop(expr_ty node, PyArena *arena,
  ...
  if (node->v.BinOp.op == Mod && rhs->kind == Tuple_kind &&
      PyUnicode_Check(lv) && !has_starred(rhs->v.Tuple.elts)) {
...
static PyObject *make_const_tuple(asdl_expr_seq *elts) {
...
*/
static int fold_iter(expr_ty arg, PyArena *arena, _PyASTOptimizeState *state) {
  PyObject *newval;
...
static int astfold_body(asdl_stmt_seq *stmts, PyArena *ctx_,
                        _PyASTOptimizeState *state) {
...
static int astfold_expr(expr_ty node_, PyArena *ctx_,
  ...
  case ListComp_kind:
    ...
    CALL_SEQ(astfold_comprehension, comprehension,
...
int _PyAST_Optimize(mod_ty mod, PyArena *arena, int optimize, int ff_features) {
