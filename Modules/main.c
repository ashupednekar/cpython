...
static int pymain_exit_err_print(void) {
  int exitcode = 1;
...
/* Write an exitcode into *exitcode and return 1 if we have to exit Python.
   Return 0 otherwise. */
static int pymain_get_importer(const wchar_t *filename, PyObject **importer_p,
                               int *exitcode) {
...
static int pymain_sys_path_add_path0(PyInterpreterState *interp,
                                     PyObject *path0) {
  PyObject *sys_path;
...
static int pymain_run_command(wchar_t *command) {
  ...
  ret = _PyRun_SimpleStringFlagsWithName(PyBytes_AsString(bytes), "<string>",
                                         &cf);
  Py_DECREF(bytes);
  return (ret != 0);
...
static int pymain_run_module(const wchar_t *modname, int set_argv0) {
...
static int pymain_run_startup(PyConfig *config, int *exitcode) {
  int ret;
...
static void pymain_set_inspect(PyConfig *config, int inspect) {
  config->inspect = inspect;
  _Py_COMP_DIAG_PUSH _Py_COMP_DIAG_IGNORE_DEPR_DECLS Py_InspectFlag = inspect;
  _Py_COMP_DIAG_POP
}

static int pymain_run_stdin(PyConfig *config) {
...
static void pymain_repl(PyConfig *config, int *exitcode) {
  /* Check this environment variable at the end, to give programs the
     opportunity to set it from Python. */
  if (!config->inspect &&
      _Py_GetEnv(config->use_environment, "PYTHONINSPECT")) {
    pymain_set_inspect(config, 1);
  }

  if (!(config->inspect && stdin_is_interactive(config) &&
        config_run_code(config))) {
    return;
  }
...
static void _Py_NO_RETURN pymain_exit_error(PyStatus status) {
